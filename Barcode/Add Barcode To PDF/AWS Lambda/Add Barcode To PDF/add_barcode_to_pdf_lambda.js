//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Add Barcode to PDF                                        //
...
const URL = `${BASE_URL}api/v2/addbarcode`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Add_barcode_to_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for barcode addition
 ...
 */
async function addBarcodeToPdf(base64Content, filename, barcodeConfig = {}) {
    const headers = {
    ...
    const payload = {
        ...
        text: barcodeConfig.text || "PDF4me Barcode Sample", // Text to encode in barcode
        barcodeType: barcodeConfig.barcodeType || "qrCode", // Barcode types: qrCode, code128, dataMatrix, aztec, hanXin, pdf417, etc.
        pages: barcodeConfig.pages || "1-3",     // Page options: "", "1", "1,3,5", "2-5", "1,3,7-10", "2-" (empty = all pages)
        alignX: barcodeConfig.alignX || "Right", // Horizontal alignment: "Left", "Center", "Right"
        alignY: barcodeConfig.alignY || "Bottom", // Vertical alignment: "Top", "Middle", "Bottom"
        heightInMM: barcodeConfig.heightInMM || "40", // Height in millimeters (string, "0" for auto-detect)
        widthInMM: barcodeConfig.widthInMM || "40", // Width in millimeters (string, "0" for auto-detect)
        marginXInMM: barcodeConfig.marginXInMM || "20", // Horizontal margin in millimeters (string)
        marginYInMM: barcodeConfig.marginYInMM || "20", // Vertical margin in millimeters (string)
        heightInPt: barcodeConfig.heightInPt || "113", // Height in points (string, "0" for auto-detect)
        widthInPt: barcodeConfig.widthInPt || "113", // Width in points (string, "0" for auto-detect)
        marginXInPt: barcodeConfig.marginXInPt || "57", // Horizontal margin in points (string)
        marginYInPt: barcodeConfig.marginYInPt || "57", // Vertical margin in points (string)
        opacity: barcodeConfig.opacity || 100,   // Opacity (0-100): 0=transparent, 100=opaque
        displayText: barcodeConfig.displayText || "below", // Text display: "above", "below"
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Barcode addition completed successfully!');
...
/**
 * Handle API response and save the PDF file with barcode
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            ...
            console.log(`PDF with barcode saved successfully: ${outputPdfPath}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Add Barcode to PDF Process in Lambda');
        ...
        console.log(`Output PDF with barcode: ${outputPdfName}`);
        console.log(`Barcode configuration:`, barcodeConfig);
        console.log('Operation: Add barcode to PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await addBarcodeToPdf(base64Content, path.basename(inputPdfPath), barcodeConfig);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('Barcode addition completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
            ...
            console.log(`Barcode addition failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Add Barcode to PDF Process (Local Test)');
        console.log('=== Adding Barcode to PDF Document ===');
        ...
        console.error(`Barcode addition failed: ${err.message}`);
        process.exit(1);
    }
}
