...
const API_URL = `${BASE_URL}api/v2/ReadSwissQR`;
const INPUT_FILE = 'sample.pdf'; // Path to the Swiss QR PDF file
const OUTPUT_FILE = 'read_swissqr_code_output.json'; // Output file for Swiss QR data

// Main function to read Swiss QR code from PDF (handles retry logic and orchestration)
async function readSwissQRCode() {
    const max_retries = 10; // Number of times to retry if it fails
    const retry_delay = 10; // Wait 10 seconds between retries
    
    try {
        console.log('Reading Swiss QR code from PDF...');
        ...
        for (let attempt = 1; attempt <= max_retries; attempt++) {
            try {
                console.log(`Attempt ${attempt}/${max_retries} - Reading Swiss QR code from PDF...`);
                
                // Process the PDF file to read Swiss QR code
                const response = await processSwissQRReading();
        ...
        console.error('Error reading Swiss QR code from PDF:', error.message);
        throw error; // Re-throw the error so the program knows it failed
    }
}

// Function to process PDF files and read Swiss QR code
async function processSwissQRReading() {
    ...
    console.log("Sending Swiss QR reading request to PDF4me API...");
    
    // Make the API request to read Swiss QR code from the PDF
    try {
        const response = await fetch(API_URL, {
            method: 'POST', // HTTP method to read Swiss QR code
...
async function handleAPIResponse(response, outputPath) {
    if (response.status === 200) {
        ...
        console.log("✓ Success! Swiss QR reading completed!");
        ...
        try {
            ...
            try {
                swissqrData = JSON.parse(responseText);
            } catch (e) {
                swissqrData = responseText;
            }
            
            // Save the Swiss QR data to JSON file
            if (typeof swissqrData === 'object') {
                fs.writeFileSync(outputPath, JSON.stringify(swissqrData, null, 2), 'utf8');
            } else {
                fs.writeFileSync(outputPath, String(swissqrData), 'utf8');
            }
            
            console.log(`Swiss QR data saved: ${outputPath}`);
            
            // Display found Swiss QR data
            if (swissqrData && typeof swissqrData === 'object') {
            ...
            console.error(`Error processing Swiss QR data: ${error.message}`);
...
async function pollForCompletion(response, outputPath) {
    ...
    const max_retries = 20; // Check up to 20 times (as per Python version)
...
// Main function to run the Swiss QR code reading
async function main() {
    try {
        // Start the Swiss QR code reading process
        await readSwissQRCode();
        
    } catch (error) {
        // If anything goes wrong, show the error and exit
        console.error('Swiss QR code reading failed:', error.message);
