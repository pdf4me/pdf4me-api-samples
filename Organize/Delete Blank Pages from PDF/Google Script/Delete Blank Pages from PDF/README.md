# Delete Blank Pages from PDF - Google Apps Script

This Google Apps Script automatically removes blank pages from PDF documents using the PDF4me API. The script processes PDF files stored in Google Drive and saves the cleaned versions to a specified output folder.

## Features

- **Automatic blank page detection**: Removes pages with no text, no images, or both
...
- **Flexible configuration**: Multiple deletion options available
...
var deletePageOption = 'NoTextNoImages'; // <-- Set deletion option
```
...
- **`NoTextNoImages`**: Removes pages that have neither text nor images
- **`NoText`**: Removes pages that have no text (may keep pages with images)
- **`NoImages`**: Removes pages that have no images (may keep pages with text)
...
3. Run the `deleteBlankPagesFromPdf()` function
...
4. Run the `deleteBlankPagesFromPdf()` function
...
4. **Processing**: PDF4me analyzes and removes blank pages based on your criteria
...
- **Processed PDF**: Saved with `.no_blank_pages.pdf` suffix
...
└── PDF4ME output/
    ├── sample.no_blank_pages.pdf (processed file)
    └── deletion_error.txt (if errors occur)
```

## API Reference

This script uses the PDF4me DeleteBlankPages API endpoint:
- **Endpoint**: `https://api.pdf4me.com/api/v2/DeleteBlankPages`
- **Method**: POST
