//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Blank Pages Deletion                                  //
...
const URL = `${BASE_URL}api/v2/DeleteBlankPages`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Delete_blank_pages_from_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for blank page deletion
 * @param {string} base64Content - Base64 encoded PDF content
 * @param {string} filename - Name of the source PDF file
 * @param {string} deletePageOption - Option for blank page detection
 ...
 */
async function deleteBlankPagesFromPdf(base64Content, filename, deletePageOption = 'NoTextNoImages') {
    const headers = {
...
async function handler(event, context) {
    try {
        console.log('Starting PDF Blank Pages Deletion Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const deletePageOption = event.deletePageOption || 'NoTextNoImages';

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output processed PDF: ${outputPdfName}`);
        console.log(`Delete page option: ${deletePageOption}`);
        console.log('Operation: Delete blank pages from PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await deleteBlankPagesFromPdf(base64Content, path.basename(inputPdfPath), deletePageOption);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('PDF blank pages deletion completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    ...
                    deletePageOption: deletePageOption
                })
            };
        } else {
            console.log(`PDF blank pages deletion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting PDF Blank Pages Deletion Process (Local Test)');
        console.log('=== Deleting Blank Pages from PDF ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputPdfName = OUTPUT_PATH;
        const deletePageOption = 'NoTextNoImages';
        ...
        if (result.status === 'success') {
            ...
            console.log('Blank pages have been removed from the PDF');
            console.log('The processed document contains only pages with content');
        } else {
            console.log(`PDF blank pages deletion failed: ${result.error}`);
            console.log('Please check your input file and API configuration');
        }
    } catch (err) {
        console.error(`PDF blank pages deletion failed: ${err.message}`);
        process.exit(1);
    }
}
