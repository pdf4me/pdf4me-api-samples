//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Unwanted Pages Deletion                               //
...
const URL = `${BASE_URL}api/v2/DeletePages`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Delete_unwanted_pages_from_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for unwanted page deletion
 * @param {string} base64Content - Base64 encoded PDF content
 * @param {string} filename - Name of the source PDF file
 * @param {string} pageNumbers - Page numbers to delete (e.g. "2-4", "1,3,5")
 ...
 */
async function deleteUnwantedPagesFromPdf(base64Content, filename, pageNumbers = '2-4') {
    const headers = {
...
async function handler(event, context) {
    try {
        console.log('Starting PDF Unwanted Pages Deletion Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const pageNumbers = event.pageNumbers || '2-4';

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output processed PDF: ${outputPdfName}`);
        console.log(`Page numbers to delete: ${pageNumbers}`);
        console.log('Operation: Delete unwanted pages from PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await deleteUnwantedPagesFromPdf(base64Content, path.basename(inputPdfPath), pageNumbers);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('PDF unwanted pages deletion completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'PDF unwanted pages deletion completed successfully',
            ...
            console.log(`PDF unwanted pages deletion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: false,
                    message: 'PDF unwanted pages deletion failed',
                    error: result.error
                })
            };
        }
...
async function main() {
    try {
        console.log('Starting PDF Unwanted Pages Deletion Process (Local Test)');
        ...
        if (result.status === 'success') {
            ...
            console.log('Unwanted pages have been removed from the PDF');
            console.log('The processed document contains only the desired pages');
        } else {
            console.log(`PDF unwanted pages deletion failed: ${result.error}`);
            console.log('Please check your input file and API configuration');
        }
    } catch (err) {
        console.error(`PDF unwanted pages deletion failed: ${err.message}`);
        process.exit(1);
    }
}
