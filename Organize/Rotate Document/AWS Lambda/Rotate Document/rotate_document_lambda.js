//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Document Rotation                                      //
...
// API Configuration - PDF4me service for rotating entire PDF documents
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for rotating entire PDF documents
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/Rotate`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Rotate_document_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for document rotation
 ...
 */
async function rotateDocument(base64Content, filename, rotationType = 'UpsideDown') {
    const headers = {
...
async function handler(event, context) {
    try {
        ...
        if (result.status === 'success') {
            ...
            console.log(`PDF document rotation failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting PDF Document Rotation Process (Local Test)');
        console.log('=== Rotating Entire PDF Document ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputPdfName = OUTPUT_PATH;
        const rotationType = 'UpsideDown';
        ...
        if (result.status === 'success') {
            ...
            console.log('All pages in the document have been rotated');
        } else {
            console.log(`PDF document rotation failed: ${result.error}`);
            console.log('Please check your input file and API configuration');
        }
    } catch (err) {
        console.error(`PDF document rotation failed: ${err.message}`);
        process.exit(1);
    }
}
