...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            String result = rotateDocument(pdfPath);
            if (result != null && !result.isEmpty()) {
                System.out.println("Output saved: " + result);
            } else {
                System.out.println("Document rotation failed.");
            }
        } catch (Exception e) {
    ...
    public static String rotateDocument(String inputPdfPath) {
        try {
            if (!Files.exists(Paths.get(inputPdfPath))) {
                System.out.println("PDF not found: " + inputPdfPath);
                return null;
            }
            String outputFileName = "Rotate_document_PDF_output.pdf";
    ...
    private static String executeDocumentRotation(Map<String, Object> payload, String outputPdfPath) {
        try {
            String jsonPayload = convertToJson(payload);
            HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + "api/v2/Rotate"))
            ...
            System.err.println("Error in executeDocumentRotation: " + ex.getMessage());
            return null;
        }
    }
