function rotateDocument() {
  ...
  var rotationType = 'UpsideDown'; // <-- Set rotation type (NoRotation, Clockwise, CounterClockwise, UpsideDown)
  ...
  try {
    ...
    // What PDF4me document rotation does:
    // - Rotates all pages in a document with the same rotation direction
    // - Supports multiple rotation types (NoRotation, Clockwise, CounterClockwise, UpsideDown)
    // - Maintains document structure and formatting
    // - Corrects document orientation for better viewing and printing
    var payload = {
    ...
    // Send the initial document rotation request to the API
    Logger.log('Sending document rotation request to PDF4me API...');
    Logger.log('Processing document rotation: ' + fileName);
    ...
    if (code === 200) {
      // 200 means "Success" - Document rotation completed successfully
      Logger.log('Success! Document rotation completed!');
      ...
      // 202 means "Accepted" - API is processing the document rotation asynchronously
      ...
      // Poll the API until document rotation is complete
      ...
      Logger.log('Timeout: Document rotation did not complete after multiple retries');
...
function processRotatedPdf(rotatedPdfBlob, outputFolderName, fileName) {
  ...
  try {
    ...
    var outputFileName = baseName + '.rotated.pdf';
    ...
    Logger.log('Document rotation completed successfully!');
    
  } catch (e) {
    Logger.log('Error processing PDF: ' + e);
    
    // Create error file
    var errorContent = 'Document Rotation Error\n' +
                      '======================\n' +
