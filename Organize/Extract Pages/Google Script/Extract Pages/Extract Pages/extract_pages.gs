function extractPages() {
  ...
  var url = `${baseUrl}api/v2/Extract`;
  ...
  var pageNumbers = '1,3'; // <-- Set page numbers to extract (e.g. "1" or "1,3,5" or "2-4")
  ...
  try {
    ...
    // What PDF4me page extraction does:
    // - Extracts specified pages from PDF documents to create shorter versions
    // - Supports single pages, multiple pages, and page ranges
    // - Maintains document structure and formatting
    // - Creates digital booklets or focused content from larger documents
    var payload = {
    ...
    // Send the initial page extraction request to the API
    Logger.log('Sending page extraction request to PDF4me API...');
    Logger.log('Processing page extraction: ' + fileName);
    Logger.log('Pages to extract: ' + pageNumbers);
    ...
    if (code === 200) {
      // 200 means "Success" - Page extraction completed successfully
      Logger.log('Success! Page extraction completed!');
      
      // Save the extracted PDF
      try {
        // Get the binary content from the response
        var extractedPdfBlob = response.getBlob();
        
        // Process and save extracted PDF
        processExtractedPdf(extractedPdfBlob, outputFolderName, file.getName());
      ...
      // 202 means "Accepted" - API is processing the page extraction asynchronously
      ...
      // Poll the API until page extraction is complete
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          try {
            ...
            var extractedPdfBlob = responseExtraction.getBlob();
      ...
      Logger.log('Timeout: Page extraction did not complete after multiple retries');
...
function processExtractedPdf(extractedPdfBlob, outputFolderName, fileName) {
  // Process and save extracted PDF
  try {
    var outputFolders = DriveApp.getFoldersByName(outputFolderName);
    if (!outputFolders.hasNext()) {
    ...
    var outputFileName = baseName + '.extracted_pages.pdf';
    
    // Set the MIME type for PDF
    extractedPdfBlob.setContentType('application/pdf');
    
    // Save the extracted PDF file
    var savedFile = outputFolder.createFile(extractedPdfBlob);
    savedFile.setName(outputFileName);
    
    Logger.log('Extracted PDF saved: ' + outputFileName);
    Logger.log('Extracted file size: ' + savedFile.getSize() + ' bytes');
    Logger.log('Page extraction completed successfully!');
    
  } catch (e) {
    Logger.log('Error processing PDF: ' + e);
    
    // Create error file
    var errorContent = 'Page Extraction Error\n' +
                      '====================\n' +
