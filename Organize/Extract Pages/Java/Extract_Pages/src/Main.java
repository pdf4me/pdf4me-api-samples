...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            String result = extractPagesFromPdf(pdfPath);
    ...
    public static String extractPagesFromPdf(String inputPdfPath) {
        try {
            if (!Files.exists(Paths.get(inputPdfPath))) {
                System.out.println("PDF not found: " + inputPdfPath);
                return null;
            }
            String outputFileName = "Extract_pages_PDF_output.pdf";
    ...
    private static String executePageExtraction(Map<String, Object> payload, String outputPdfPath) {
        try {
            String jsonPayload = convertToJson(payload);
            HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + "api/v2/Extract"))
            ...
            System.err.println("Error in executePageExtraction: " + ex.getMessage());
            return null;
        }
    }
