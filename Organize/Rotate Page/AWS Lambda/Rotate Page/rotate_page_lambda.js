//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Page Rotation                                          //
...
const URL = `${BASE_URL}api/v2/RotatePage`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Rotate_page_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for page rotation
 * @param {string} base64Content - Base64 encoded PDF content
 * @param {string} filename - Name of the source PDF file
 * @param {string} rotationType - Rotation type: NoRotation, Clockwise, CounterClockwise, UpsideDown
 * @param {string} page - Page numbers to rotate (e.g. "1", "1,3,5", "2-4")
 ...
 */
async function rotatePage(base64Content, filename, rotationType = 'Clockwise', page = '1') {
    const headers = {
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Pages rotated successfully!');
...
async function handler(event, context) {
    try {
        console.log('Starting PDF Page Rotation Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const rotationType = event.rotationType || 'Clockwise';
        const page = event.page || '1';

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output processed PDF: ${outputPdfName}`);
        console.log(`Rotation type: ${rotationType}`);
        console.log(`Page(s) to rotate: ${page}`);
        console.log('Operation: Rotate specific pages in PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await rotatePage(base64Content, path.basename(inputPdfPath), rotationType, page);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('PDF page rotation completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
            ...
            console.log(`PDF page rotation failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting PDF Page Rotation Process (Local Test)');
        console.log('=== Rotating Specific Pages in PDF Document ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputPdfName = OUTPUT_PATH;
        const rotationType = 'Clockwise';
        const page = '1';
        ...
        if (result.status === 'success') {
            ...
            console.log('Specified pages in the document have been rotated');
        } else {
            console.log(`PDF page rotation failed: ${result.error}`);
            console.log('Please check your input file and API configuration');
        }
    } catch (err) {
        console.error(`PDF page rotation failed: ${err.message}`);
        process.exit(1);
    }
}
