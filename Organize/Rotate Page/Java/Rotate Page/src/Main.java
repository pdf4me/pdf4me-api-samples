...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            String result = rotatePage(pdfPath);
            if (result != null && !result.isEmpty()) {
                System.out.println("Output saved: " + result);
            } else {
                System.out.println("Page rotation failed.");
            }
        } catch (Exception e) {
    ...
    public static String rotatePage(String inputPdfPath) {
        try {
            if (!Files.exists(Paths.get(inputPdfPath))) {
                System.out.println("PDF not found: " + inputPdfPath);
                return null;
            }
            String outputFileName = "Rotate_page_PDF_output.pdf";
    ...
    private static String executePageRotation(Map<String, Object> payload, String outputPdfPath) {
        try {
            String jsonPayload = convertToJson(payload);
            HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + "api/v2/RotatePage"))
            ...
            System.err.println("Error in executePageRotation: " + ex.getMessage());
            return null;
        }
    }
