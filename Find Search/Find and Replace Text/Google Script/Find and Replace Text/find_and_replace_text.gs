function findAndReplaceText() {
  ...
  var url = `${baseUrl}api/v2/FindAndReplace`;
  ...
  var outputFileName = 'find_and_replace_PDF_output.pdf';
  ...
  try {
    ...
    // - Supports page-specific text replacement
    // - Maintains document formatting and layout
    // - Useful for updating documents, correcting errors, and batch text modifications
    var payload = {
      docContent: pdfBase64,                        // Base64 encoded PDF document content
      docName: file.getName(),                      // Source PDF file name with .pdf extension
      oldText: oldText,                             // Text to be searched (Required)
      newText: newText,                             // Text to be replaced with (Required)
      pageSequence: pageSequence,                   // Page indices (Required):
    ...
    // Send the initial find and replace request to the API
    Logger.log('Sending PDF to PDF4me API for find and replace operation...');
    Logger.log('Processing find and replace: ' + fileName + ' → ' + outputFileName);
    ...
    if (code === 200) {
      // 200 means "Success" - find and replace completed successfully
      Logger.log(' Success! Text replacement completed successfully!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var processedPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(processedPdfBlob);
        Logger.log('Find and replace PDF saved to: ' + outputFileName);
        Logger.log('Text replacement completed successfully!');
        Logger.log('Text "' + oldText + '" replaced with "' + newText + '"');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the find and replace asynchronously
      ...
      // Poll the API until find and replace is complete
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          Logger.log(' Success! Text replacement completed!');
          ...
          if (responseText.startsWith('%PDF') || responseBlob.getBytes().length > 1000) {
            var processedPdfBlob = responseBlob.setName(outputFileName);
