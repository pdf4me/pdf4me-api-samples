//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Extract Attachment From PDF                                //
...
// API Configuration - PDF4me service for extracting attachments from PDF
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for extracting attachments from PDF
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ExtractAttachmentFromPdf`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Extracted_attachments.zip";
...
/**
 * Send PDF to PDF4me API for attachment extraction
 ...
 * @returns {Promise<Object>} API response containing the extracted attachments or processing status
 * @throws {Error} For API request errors
 */
async function extractAttachmentFromPdf(pdfBase64, filename, extractionConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for attachment extraction...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Attachment extraction completed successfully!');
...
/**
 * Handle API response and save the extracted attachments
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputZipPath - Path where to save the extracted attachments
...
async function handleAsyncResponse(apiResponse, outputZipPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary attachment extraction response directly...');
            fs.writeFileSync(outputZipPath, Buffer.from(apiResponse.binaryContent));
            console.log(`Attachments extracted and saved successfully: ${outputZipPath}`);
        ...
        console.error(`Error saving extracted attachments: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Extract Attachment From PDF Process in Lambda');
        ...
        console.log('Operation: Extract Attachment From PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await extractAttachmentFromPdf(base64Content, path.basename(inputPdfPath), extractionConfig);
        ...
        if (result.status === 'success') {
            ...
            console.log(`Attachment extraction failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Extract Attachment From PDF Process (Local Test)');
        ...
        console.error(`Attachment extraction failed: ${err.message}`);
        process.exit(1);
    }
}
