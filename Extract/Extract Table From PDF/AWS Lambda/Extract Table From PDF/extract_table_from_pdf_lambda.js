//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Extract Table From PDF                                     //
...
// API Configuration - PDF4me service for extracting tables from PDF
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for extracting tables from PDF
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ExtractTableFromPdf`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Extracted_table.json";
...
/**
 * Send PDF to PDF4me API for table extraction
 ...
 * @returns {Promise<Object>} API response containing the extracted table data or processing status
 * @throws {Error} For API request errors
 */
async function extractTableFromPdf(pdfBase64, filename, extractionConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for table extraction...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Table extraction completed successfully!');
            return { tableData: response.data, status: 'success' };
...
/**
 * Handle API response and save the extracted table data
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputJsonPath - Path where to save the extracted table data
...
async function handleAsyncResponse(apiResponse, outputJsonPath) {
    try {
        if (apiResponse.tableData && apiResponse.status === 'success') {
            console.log('Processing table extraction results directly...');
            fs.writeFileSync(outputJsonPath, JSON.stringify(apiResponse.tableData, null, 2));
            console.log(`Table data extracted and saved successfully: ${outputJsonPath}`);
            return { outputPath: outputJsonPath, status: 'success', tableData: apiResponse.tableData };
                ...
                for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                    ...
                    try {
                        ...
                        if (statusResponse.status === 200) {
                            ...
                            const tableData = statusResponse.data;
                            fs.writeFileSync(outputJsonPath, JSON.stringify(tableData, null, 2));
                            console.log(`Table data extracted and saved successfully: ${outputJsonPath}`);
                            return { outputPath: outputJsonPath, status: 'success', tableData: tableData };
        ...
        console.error(`Error saving extracted table data: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Extract Table From PDF Process in Lambda');
        ...
        console.log('Operation: Extract Table From PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await extractTableFromPdf(base64Content, path.basename(inputPdfPath), extractionConfig);
        ...
        if (result.status === 'success') {
            ...
            console.log(`Table extraction failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Extract Table From PDF Process (Local Test)');
        console.log('=== Extracting Table From PDF ===');
        ...
        console.error(`Table extraction failed: ${err.message}`);
        process.exit(1);
    }
}
