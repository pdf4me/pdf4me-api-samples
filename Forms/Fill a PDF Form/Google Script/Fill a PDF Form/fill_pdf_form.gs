function fillPdfForm() {
  ...
  var url = `${baseUrl}api/v2/FillPdfForm`;
  ...
  var outputFileName = 'fill_PDF_form_output.pdf';
  ...
  try {
    ...
    // What filling PDF forms does:
    // - Fills existing form fields in PDF documents with provided data
    // - Supports various form field types (text, checkbox, dropdown, etc.)
    // - Maintains document formatting and layout
    // - Useful for automated form processing, data entry, and document generation
    var payload = {
      templateDocName: file.getName(),              // Template document name (Required)
    ...
    // Send the initial PDF form filling request to the API
    Logger.log('Sending PDF to PDF4me API for filling form fields...');
    Logger.log('Processing PDF form filling: ' + fileName + ' → ' + outputFileName);
    ...
    if (code === 200) {
      // 200 means "Success" - PDF form filling completed successfully
      Logger.log(' Success! PDF form filled successfully!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var filledPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(filledPdfBlob);
        Logger.log('Filled PDF form saved to: ' + outputFileName);
        Logger.log('PDF form filling completed successfully!');
        Logger.log('Filled ' + Object.keys(formData).length + ' form fields');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the PDF form filling asynchronously
      ...
      // Poll the API until PDF form filling is complete
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          Logger.log(' Success! PDF form filling completed!');
          ...
          if (responseText.startsWith('%PDF') || responseBlob.getBytes().length > 1000) {
            var filledPdfBlob = responseBlob.setName(outputFileName);
