function addFormFieldsToPdf() {
  ...
  // Set the output file name for the processed PDF
  var outputFileName = 'add_form_fields_PDF_output.pdf';
  var outputFolderName = 'PDF4ME output'; // <-- Set your output folder name here

  // Configure form field parameters
  var initialValue = "New input text";     // Initial value for form field
  var positionX = 300;                     // X position of the form field
  var positionY = 300;                     // Y position of the form field
  var fieldName = "Input Field Name";      // Name of the form field
  var fieldSize = 4;                       // Size of the form field
  var pages = "1";                         // Page indices (all pages if not specified)
  var formFieldType = "TextBox";           // Form field type: "TextBox" or "CheckBox"
  ...
  try {
    ...
    // What adding form fields does:
    // - Adds interactive form fields (TextBox, CheckBox) to PDF documents
    // - Supports positioning, sizing, and initial values for form fields
    // - Maintains document formatting and layout
    // - Useful for creating fillable forms, surveys, and interactive documents
    var payload = {
      docContent: pdfBase64,                        // Base64 encoded PDF document content
      docName: file.getName(),                      // Source PDF file name with .pdf extension
      initialValue: initialValue,                   // Initial value for form field (Required)
      positionX: positionX,                         // X position of the form field (Required)
      positionY: positionY,                         // Y position of the form field (Required)
      fieldName: fieldName,                         // Name of the form field (Required)
    ...
    // Send the initial form field addition request to the API
    Logger.log('Sending PDF to PDF4me API for adding form fields...');
    Logger.log('Processing form field addition: ' + fileName + ' → ' + outputFileName);
    ...
    if (code === 200) {
      // 200 means "Success" - form field addition completed successfully
      Logger.log(' Success! Form fields added successfully!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var formFieldsPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(formFieldsPdfBlob);
        Logger.log('Form fields PDF saved to: ' + outputFileName);
        Logger.log('Form field addition completed successfully!');
        Logger.log('Added ' + formFieldType + ' field: "' + fieldName + '"');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the form field addition asynchronously
      ...
      // Poll the API until form field addition is complete
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          Logger.log(' Success! Form field addition completed!');
          
          // Validate and save the processed PDF
          ...
          if (responseText.startsWith('%PDF') || responseBlob.getBytes().length > 1000) {
            var formFieldsPdfBlob = responseBlob.setName(outputFileName);
