...
public class Main {
    ...
    private static final String API_URL = "https://api.pdf4me.com/api/v2/ConvertWordToPdfForm";
    private static final HttpClient httpClient = HttpClient.newHttpClient();

    public static void main(String[] args) {
        String inputPath = "sample.docx";
        String outputPath = "Word_to_PDF_Form_output.pdf";
        try {
            System.out.println("Converting Word to PDF Form...");
            convertWordToPdfForm(inputPath, outputPath);
    ...
    public static void convertWordToPdfForm(String inputPath, String outputPath) throws Exception {
        ...
        if (resp.status == 200) {
            savePdfForm(resp.bytes, outputPath);
    ...
    private static void savePdfForm(byte[] content, String output) throws IOException {
        if (isPdfContent(content)) {
            Files.write(Paths.get(output), content);
            System.out.println("PDF form saved successfully to: " + output);
    ...
    private static void pollForResult(String locationUrl, String output) throws Exception {
        ...
        for (int attempt = 1; attempt <= 10; attempt++) {
            ...
            if (pollResp.status == 200) {
                savePdfForm(pollResp.bytes, output);
                return;
