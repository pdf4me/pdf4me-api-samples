//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Linearize PDF                                               //
...
// API endpoint for linearizing PDF documents
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/LinearizePdf`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "Linearized_PDF_output.pdf";
...
/**
 * Send PDF to PDF4me API for linearization
 * @param {string} pdfBase64 - Base64 encoded PDF content
 * @param {string} filename - Name of the source PDF file
 * @param {Object} linearizeConfig - Linearization configuration parameters
 ...
 */
async function linearizePdf(pdfBase64, filename, linearizeConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for linearization...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! PDF linearization completed successfully!');
...
/**
 * Handle API response and save the linearized PDF file
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputPdfPath - Path where to save the processed linearized PDF
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary linearized PDF response directly...');
            fs.writeFileSync(outputPdfPath, Buffer.from(apiResponse.binaryContent));
            console.log(`Linearized PDF saved successfully: ${outputPdfPath}`);
        ...
        console.error(`Error saving linearized PDF: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Linearize PDF Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const linearizeConfig = event.linearizeConfig || {};

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output Linearized PDF: ${outputPdfName}`);
        console.log(`Linearization configuration:`, linearizeConfig);
        console.log('Operation: Linearize PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await linearizePdf(base64Content, path.basename(inputPdfPath), linearizeConfig);
        ...
        if (result.status === 'success') {
            ...
            console.log(`PDF linearization failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Linearize PDF Process (Local Test)');
        console.log('=== Linearizing PDF Document ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputPdfName = OUTPUT_PATH;
        const linearizeConfig = {};
        ...
        if (result.status === 'success') {
            ...
            console.log(`Linearization configuration used:`, linearizeConfig);
        } else {
            console.log(`PDF linearization failed: ${result.error}`);
            console.log('Please check your input file and API configuration');
        }
    } catch (err) {
        console.error(`PDF linearization failed: ${err.message}`);
        process.exit(1);
    }
}
