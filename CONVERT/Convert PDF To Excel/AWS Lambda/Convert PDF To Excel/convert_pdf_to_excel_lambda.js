//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Convert PDF to Excel                                       //
...
// API Configuration - PDF4me service for converting PDF to Excel
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for converting PDF to Excel
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ConvertPdfToExcel`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "PDF_to_Excel_output.xlsx";
...
/**
 * Send PDF to PDF4me API for Excel conversion
 ...
 */
async function convertPdfToExcel(pdfBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for Excel conversion...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! PDF to Excel conversion completed successfully!');
...
async function handler(event, context) {
    try {
        console.log('Starting Convert PDF to Excel Process in Lambda');
        ...
        console.log('Operation: Convert PDF to Excel');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await convertPdfToExcel(base64Content, path.basename(inputPdfPath), conversionConfig);
        ...
        if (result.status === 'success') {
            ...
            console.log(`PDF to Excel conversion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Convert PDF to Excel Process (Local Test)');
        console.log('=== Converting PDF Document to Excel ===');
        ...
        console.error(`PDF to Excel conversion failed: ${err.message}`);
        process.exit(1);
    }
}
