...
public class Main {
    ...
    private static final String API_URL = "https://api.pdf4me.com/api/v2/FlattenPdf";
    private static final HttpClient httpClient = HttpClient.newHttpClient();

    public static void main(String[] args) {
        String inputPath = "unflattened-sample.pdf";
        String outputPath = "Flatten_PDF_output.pdf";
        try {
    ...
    public static void flattenPdf(String inputPath, String outputPath) throws Exception {
        ...
        if (resp.status == 200) {
            saveFlattenedPdf(resp.bytes, outputPath);
    ...
    private static void saveFlattenedPdf(byte[] content, String output) throws IOException {
        if (isPdfContent(content)) {
            Files.write(Paths.get(output), content);
            System.out.println("Flattened PDF saved successfully to: " + output);
            System.out.println("File size: " + content.length + " bytes");
            System.out.println("All interactive elements have been converted to static content");
        } else {
    ...
    private static void pollForResult(String locationUrl, String output) throws Exception {
        ...
        for (int attempt = 1; attempt <= 10; attempt++) {
            ...
            if (pollResp.status == 200) {
                saveFlattenedPdf(pollResp.bytes, output);
                return;
