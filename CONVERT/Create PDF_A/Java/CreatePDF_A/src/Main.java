...
public class Main {
    ...
    private static final String API_URL = "https://api.pdf4me.com/api/v2/PdfA";
    private static final HttpClient httpClient = HttpClient.newHttpClient();

    public static void main(String[] args) {
        String inputPath = "sample.pdf";
        String outputPath = "PDF_to_PDF_A_output.pdf";
        try {
            System.out.println("Converting PDF to PDF/A...");
            createPdfA(inputPath, outputPath);
    ...
    public static void createPdfA(String inputPath, String outputPath) throws Exception {
        ...
        if (resp.status == 200) {
            savePdfA(resp.bytes, outputPath);
    ...
    private static void savePdfA(byte[] content, String output) throws IOException {
        if (isPdfContent(content)) {
            Files.write(Paths.get(output), content);
            System.out.println("PDF/A file saved successfully to: " + output);
            System.out.println("File size: " + content.length + " bytes");
            System.out.println("File is now compliant with PDF/A archival standards");
        } else {
    ...
    private static void pollForResult(String locationUrl, String output) throws Exception {
        ...
        for (int attempt = 1; attempt <= 10; attempt++) {
            ...
            if (pollResp.status == 200) {
                savePdfA(pollResp.bytes, output);
                return;
