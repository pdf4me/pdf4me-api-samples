//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Create PDF_A                                                //
...
// API Configuration - PDF4me service for creating PDF/A documents
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for creating PDF/A documents
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/CreatePdfA`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "PDF_A_output.pdf";
...
/**
 * Send PDF to PDF4me API for PDF/A conversion
 ...
 */
async function createPdfA(pdfBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for PDF/A conversion...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! PDF/A conversion completed successfully!');
...
/**
 * Handle API response and save the converted PDF/A file
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputPdfPath - Path where to save the processed PDF/A
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary PDF/A response directly...');
            fs.writeFileSync(outputPdfPath, Buffer.from(apiResponse.binaryContent));
            console.log(`PDF/A conversion saved successfully: ${outputPdfPath}`);
        ...
        console.error(`Error saving converted PDF/A: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Create PDF_A Process in Lambda');
        ...
        console.log(`Output PDF/A: ${outputPdfName}`);
        console.log(`Conversion configuration:`, conversionConfig);
        console.log('Operation: Create PDF/A');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await createPdfA(base64Content, path.basename(inputPdfPath), conversionConfig);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('PDF/A creation completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'PDF/A creation completed successfully',
            ...
            console.log(`PDF/A creation failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Create PDF_A Process (Local Test)');
        console.log('=== Creating PDF/A Document ===');
        ...
        console.error(`PDF/A creation failed: ${err.message}`);
        process.exit(1);
    }
}
