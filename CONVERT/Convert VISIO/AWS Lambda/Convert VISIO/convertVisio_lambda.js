//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Convert VISIO                                              //
...
// API endpoint for converting VISIO files
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ConvertVisio?schemaVal=PDF`;

// File paths configuration
const INPUT_VISIO_PATH = "sample.vsdx";
const OUTPUT_PATH = "VISIO_to_PDF_output.pdf";
...
/**
 * Read VISIO file and convert it to base64 encoding
 * @param {string} filePath - Path to the VISIO file to be processed
 * @returns {string} Base64 encoded content of the VISIO file
 * @throws {Error} If the specified file doesn't exist or reading fails
 */
function readAndEncodeVisio(filePath) {
    if (!fs.existsSync(filePath)) {
        const errMsg = `Error: VISIO file not found at ${filePath}`;
        console.error(errMsg);
        throw new Error(`VISIO file not found: ${filePath}`);
    }
    try {
        const visioContent = fs.readFileSync(filePath);
        const base64Content = Buffer.from(visioContent).toString('base64');
        console.log(`VISIO file read successfully: ${visioContent.length} bytes`);
        return base64Content;
    } catch (err) {
        const errMsg = `Error reading VISIO file: ${err.message}`;
        console.error(errMsg);
        throw err;
    }
}
...
/**
 * Send VISIO file to PDF4me API for PDF conversion
 * @param {string} visioBase64 - Base64 encoded VISIO content
 * @param {string} filename - Name of the source VISIO file
 ...
 */
async function convertVisioToPdf(visioBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! VISIO to PDF conversion completed successfully!');
...
async function handler(event, context) {
    try {
        console.log('Starting Convert VISIO Process in Lambda');
        const inputVisioPath = event.inputVisioPath || 'sample.vsdx';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const conversionConfig = event.conversionConfig || {};

        console.log(`Input VISIO: ${inputVisioPath}`);
        console.log(`Output PDF: ${outputPdfName}`);
        console.log(`Conversion configuration:`, conversionConfig);
        console.log('Operation: Convert VISIO to PDF');

        const base64Content = readAndEncodeVisio(inputVisioPath);
        const apiResponse = await convertVisioToPdf(base64Content, path.basename(inputVisioPath), conversionConfig);
        ...
        if (result.status === 'success') {
            ...
            return {
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'VISIO to PDF conversion completed successfully',
            ...
            console.log(`VISIO to PDF conversion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: false,
                    message: 'VISIO to PDF conversion failed',
                    error: result.error
                })
            };
        }
...
async function main() {
    try {
        console.log('Starting Convert VISIO Process (Local Test)');
        console.log('=== Converting VISIO Document to PDF ===');
        
        const inputVisioPath = INPUT_VISIO_PATH;
        ...
        if (result.status === 'success') {
            console.log('VISIO to PDF conversion completed successfully!');
            console.log(`Input file: ${inputVisioPath}`);
        ...
        console.error(`VISIO to PDF conversion failed: ${err.message}`);
        process.exit(1);
    }
}
