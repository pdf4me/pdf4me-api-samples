//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Convert To PDF                                            //
...
// API endpoint for converting documents to PDF
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ConvertToPdf`;

// File paths configuration
const INPUT_DOC_PATH = "sample.txt";
const OUTPUT_PATH = "Document_to_PDF_output.pdf";
...
/**
 * Read document file and convert it to base64 encoding
 * @param {string} filePath - Path to the document file to be processed
 * @returns {string} Base64 encoded content of the document file
 * @throws {Error} If the specified file doesn't exist or reading fails
 */
function readAndEncodeDocument(filePath) {
    if (!fs.existsSync(filePath)) {
        const errMsg = `Error: Document file not found at ${filePath}`;
        console.error(errMsg);
        throw new Error(`Document file not found: ${filePath}`);
    }
    try {
        const docContent = fs.readFileSync(filePath);
        const base64Content = Buffer.from(docContent).toString('base64');
        console.log(`Document file read successfully: ${docContent.length} bytes`);
        return base64Content;
    } catch (err) {
        const errMsg = `Error reading document file: ${err.message}`;
        console.error(errMsg);
        throw err;
    }
}
...
/**
 * Send document to PDF4me API for PDF conversion
 * @param {string} docBase64 - Base64 encoded document content
 * @param {string} filename - Name of the source document file
 ...
 */
async function convertToPdf(docBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Document to PDF conversion completed successfully!');
...
async function handler(event, context) {
    try {
        console.log('Starting Convert To PDF Process in Lambda');
        const inputDocPath = event.inputDocPath || 'sample.txt';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const conversionConfig = event.conversionConfig || {};

        console.log(`Input Document: ${inputDocPath}`);
        console.log(`Output PDF: ${outputPdfName}`);
        console.log(`Conversion configuration:`, conversionConfig);
        console.log('Operation: Convert Document to PDF');

        const base64Content = readAndEncodeDocument(inputDocPath);
        const apiResponse = await convertToPdf(base64Content, path.basename(inputDocPath), conversionConfig);
        ...
        if (result.status === 'success') {
            ...
            return {
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'Document to PDF conversion completed successfully',
            ...
            console.log(`Document to PDF conversion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: false,
                    message: 'Document to PDF conversion failed',
                    error: result.error
                })
            };
        }
...
async function main() {
    try {
        console.log('Starting Convert To PDF Process (Local Test)');
        console.log('=== Converting Document to PDF ===');
        
        const inputDocPath = INPUT_DOC_PATH;
        ...
        if (result.status === 'success') {
            console.log('Document to PDF conversion completed successfully!');
            console.log(`Input file: ${inputDocPath}`);
