//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Convert To Powerpoint                                      //
...
// API Configuration - PDF4me service for converting documents to Powerpoint
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for converting documents to Powerpoint
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ConvertPdfToPowerPoint`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "PDF_to_Powerpoint_output.pptx";
...
/**
 * Send PDF to PDF4me API for Powerpoint conversion
 ...
 */
async function convertToPowerpoint(pdfBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    console.log('Sending PDF to PDF4me API for Powerpoint conversion...');
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! PDF to Powerpoint conversion completed successfully!');
...
/**
 * Handle API response and save the converted Powerpoint file
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputPowerpointPath - Path where to save the processed Powerpoint file
 * @returns {Promise<Object>} Object containing output path and status
 */
async function handleAsyncResponse(apiResponse, outputPowerpointPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary Powerpoint response directly...');
            fs.writeFileSync(outputPowerpointPath, Buffer.from(apiResponse.binaryContent));
            console.log(`Powerpoint conversion saved successfully: ${outputPowerpointPath}`);
            console.log(`Output file size: ${apiResponse.binaryContent.length} bytes`);
            return { outputPath: outputPowerpointPath, status: 'success', fileSize: apiResponse.binaryContent.length };
                ...
                for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                    ...
                    try {
                        ...
                        if (statusResponse.status === 200) {
                            ...
                            fs.writeFileSync(outputPowerpointPath, Buffer.from(statusResponse.data));
                            console.log(`Powerpoint conversion saved successfully: ${outputPowerpointPath}`);
                            console.log(`Output file size: ${statusResponse.data.length} bytes`);
                            return { outputPath: outputPowerpointPath, status: 'success', fileSize: statusResponse.data.length };
        ...
        console.error(`Error saving converted Powerpoint file: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Convert To Powerpoint Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPowerpointName = event.outputPowerpointName || OUTPUT_PATH;
        const conversionConfig = event.conversionConfig || {};

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output Powerpoint: ${outputPowerpointName}`);
        console.log(`Conversion configuration:`, conversionConfig);
        console.log('Operation: Convert PDF to Powerpoint');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await convertToPowerpoint(base64Content, path.basename(inputPdfPath), conversionConfig);
        const result = await handleAsyncResponse(apiResponse, outputPowerpointName);
        ...
        if (result.status === 'success') {
            ...
            return {
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'PDF to Powerpoint conversion completed successfully',
                    outputFileName: outputPowerpointName,
                    fileSize: result.fileSize,
                    conversionConfig: conversionConfig
                })
            };
        } else {
            console.log(`PDF to Powerpoint conversion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Convert To Powerpoint Process (Local Test)');
        console.log('=== Converting PDF Document to Powerpoint ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputPowerpointName = OUTPUT_PATH;
        ...
        if (result.status === 'success') {
            ...
            console.log(`Output file: ${outputPowerpointName}`);
        ...
        console.error(`PDF to Powerpoint conversion failed: ${err.message}`);
        process.exit(1);
    }
}
