//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Convert PDF to Word                                        //
...
// API Configuration - PDF4me service for converting PDF to Word
...
const URL = `${BASE_URL}api/v2/ConvertPdfToWord`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "PDF_to_Word_output.docx";
...
/**
 * Send PDF to PDF4me API for Word conversion
 ...
 */
async function convertPdfToWord(pdfBase64, filename, conversionConfig = {}) {
    const headers = {
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! PDF to Word conversion completed successfully!');
...
/**
 * Handle API response and save the converted Word file
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputWordPath - Path where to save the processed Word file
 * @returns {Promise<Object>} Object containing output path and status
 */
async function handleAsyncResponse(apiResponse, outputWordPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary Word response directly...');
            fs.writeFileSync(outputWordPath, Buffer.from(apiResponse.binaryContent));
            console.log(`Word conversion saved successfully: ${outputWordPath}`);
            console.log(`Output file size: ${apiResponse.binaryContent.length} bytes`);
            return { outputPath: outputWordPath, status: 'success', fileSize: apiResponse.binaryContent.length };
                ...
                for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                    ...
                    try {
                        ...
                        if (statusResponse.status === 200) {
                            ...
                            fs.writeFileSync(outputWordPath, Buffer.from(statusResponse.data));
                            console.log(`Word conversion saved successfully: ${outputWordPath}`);
                            console.log(`Output file size: ${statusResponse.data.length} bytes`);
                            return { outputPath: outputWordPath, status: 'success', fileSize: statusResponse.data.length };
        ...
        console.error(`Error saving converted Word file: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Convert PDF to Word Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputWordName = event.outputWordName || OUTPUT_PATH;
        const conversionConfig = event.conversionConfig || {};

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output Word: ${outputWordName}`);
        console.log(`Conversion configuration:`, conversionConfig);
        console.log('Operation: Convert PDF to Word');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await convertPdfToWord(base64Content, path.basename(inputPdfPath), conversionConfig);
        const result = await handleAsyncResponse(apiResponse, outputWordName);
        ...
        if (result.status === 'success') {
            ...
            return {
                ...
                body: JSON.stringify({
                    ...
                    outputFileName: outputWordName,
                    fileSize: result.fileSize,
                    conversionConfig: conversionConfig
                })
            };
        } else {
            console.log(`PDF to Word conversion failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting Convert PDF to Word Process (Local Test)');
        console.log('=== Converting PDF Document to Word ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const outputWordName = OUTPUT_PATH;
        ...
        if (result.status === 'success') {
            ...
            console.log(`Output file: ${outputWordName}`);
