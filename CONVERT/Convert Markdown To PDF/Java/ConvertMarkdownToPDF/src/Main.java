 ...
 * This implementation follows the comprehensive logic from the Python version
 * using modern Java HTTP Client for robust API communication
 */
import java.io.*;
...
public class Main {
    ...
    public static void convertMarkdownToPdf(String markdown, boolean isFile, String output) throws Exception {
        // Read and encode markdown content
        String mdBase64 = isFile ? 
            Base64.getEncoder().encodeToString(Files.readAllBytes(Paths.get(markdown))) :
        ...
        if (resp.status == 200) {
            ...
            throw new IOException("API failed: " + resp.status);
        }
    }
    ...
    private static void pollForResult(String locationUrl, String output) throws Exception {
        ...
        for (int attempt = 1; attempt <= 10; attempt++) {
            ...
            if (pollResp.status == 200) {
                ...
                throw new IOException("Polling failed: " + pollResp.status);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            if (Files.exists(Paths.get(in))) {
                convertMarkdownToPdf(in, true, out);
            } else {
                convertMarkdownToPdf("# Sample Markdown\n\nThis is a **sample** markdown document.", false, out);
            }
