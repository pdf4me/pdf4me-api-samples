function getTrackingChangesInWord() {
  ...
  // Set the output file name for the tracking changes JSON
  var outputFileName = 'sample.tracking_changes.json';
...
//          ===  Set the file ID for the input Word document ===
    ...
    // Send the Word document to the API for getting tracking changes
    Logger.log('Sending get tracking changes request...');
    Logger.log('Input Word file: ' + fileName);
    Logger.log('Output JSON file: ' + outputFileName);
    Logger.log('Extracting tracking changes information...');
    
    var response = UrlFetchApp.fetch(url, options);
    var code = response.getResponseCode();
    Logger.log('API response code: ' + code);
    
    // Only log response content if there's an error (not for successful JSON data)
    ...
    if (code === 200) {
      Logger.log('Getting tracking changes completed successfully!');
      // Read the tracking changes JSON content from the response
      var resultJson = response.getContentText();
      
      // Create a text file with the JSON content
      var jsonBlob = Utilities.newBlob(resultJson, 'application/json', outputFileName);
      ...
      Logger.log('Tracking changes JSON saved to folder: ' + outputFolderName);
      Logger.log('All tracking changes have been extracted and saved as JSON');
      Logger.log('The JSON contains details about all revisions, comments, and changes');
        return;
        }
    // If processing is asynchronous, poll for the result
      ...
      for (var i = 0; i < maxRetries; i++) {
        ...
        // If processing is complete, save the JSON file
        if (pollCode === 200) {
          Logger.log('Getting tracking changes completed successfully!');
          
          // Read the tracking changes JSON content from the polling response
          var resultJson = pollResponse.getContentText();
      ...
      Logger.log('Timeout: Getting tracking changes did not complete after multiple retries.');
    } 
