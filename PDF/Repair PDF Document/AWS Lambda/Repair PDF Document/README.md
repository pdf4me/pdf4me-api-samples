# PDF4me PDF Repair - AWS Lambda & Standalone

This project provides both **standalone Node.js** and **AWS Lambda** versions of the PDF4me PDF repair functionality.

## 📁 Files Overview

- **`repair_pdf_document.js`** - Standalone Node.js script (runs locally)
- **`repair_pdf_document_lambda.js`** - AWS Lambda handler function
...
2. **Replace API key** in `repair_pdf_document.js`:
   ```javascript
...
**For Standalone:**
- Edit `repair_pdf_document.js` and replace the `API_KEY` constant
...
#### HTTP API Call
```bash
curl -X POST https://your-api-gateway-url/repair-pdf \
  -H "Content-Type: application/json" \
  -d '{
    "inputPdfPath": "sample.pdf",
    "outputPdfName": "repaired.pdf"
  }'
```

#### S3 Trigger
Upload a PDF file to the configured S3 bucket and the Lambda function will automatically process it.
...
# Test locally
curl -X POST http://localhost:3000/repair-pdf \
  -H "Content-Type: application/json" \
  -d '{
    "inputPdfPath": "sample.pdf"
  }'
...
```
├── repair_pdf_document.js          # Standalone version
├── repair_pdf_document_lambda.js   # Lambda version
...
```json
{
  ...
  "body": {
    ...
    "outputFileName": "sample.repaired.pdf",
    "fileSize": 14289,
