# PDF4me PDF Metadata Extraction - AWS Lambda & Standalone

This project provides both **standalone Node.js** and **AWS Lambda** versions of the PDF4me PDF metadata extraction functionality, based on the Python implementation.

## 📁 Files Overview

- **`get_pdf_metadata.js`** - Standalone Node.js script (runs locally)
- **`get_pdf_metadata_lambda.js`** - AWS Lambda handler function
...
2. **Replace API key** in `get_pdf_metadata_lambda.js`:
   ```javascript
...
**For Standalone:**
- Edit `get_pdf_metadata_lambda.js` and replace the `API_KEY` constant
...
# Direct execution
node get_pdf_metadata_lambda.js
```
...
#### HTTP API Call
```bash
curl -X POST https://your-api-gateway-url/get-metadata \
  -H "Content-Type: application/json" \
  -d '{
    "inputPdfPath": "sample.pdf",
    "outputMetadataName": "metadata.json"
  }'
```

#### S3 Trigger
Upload a PDF file to the configured S3 bucket and the Lambda function will automatically extract its metadata.
...
```
├── get_pdf_metadata.js              # Standalone version
├── get_pdf_metadata_lambda.js       # Lambda version
...
```json
{
  ...
  "body": {
    ...
    "outputFileName": "sample.metadata.json",
    "metadata": {
