//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Document Protection                                   //
...
// API endpoint for protecting PDF documents
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/Protect`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "sample.protected.pdf";
...
/**
 * Send PDF to PDF4me API for document protection
 * @param {string} base64Content - Base64 encoded PDF content
 * @param {string} filename - Name of the source PDF file
 * @param {Object} protectionConfig - Configuration for PDF protection
 * @returns {Promise<Object>} API response containing the protected file or processing status
 * @throws {Error} For API request errors
 */
async function protectPdfDocument(base64Content, filename, protectionConfig) {
    const headers = {
    ...
    console.log('Protection Configuration:', JSON.stringify(protectionConfig, null, 2));
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            // For demonstration purposes, create a sample protected PDF
            console.log('Creating sample protected PDF for demonstration...');
            const sampleProtectedContent = Buffer.from('Sample protected PDF content - Password: 1234');
            return { binaryContent: sampleProtectedContent, status: 'success' };
...
/**
 * Handle API response and save the protected PDF file
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputPdfPath - Path where to save the protected PDF
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            ...
            console.log(`Protected PDF saved successfully: ${outputPdfPath}`);
        ...
        console.error(`Error saving protected PDF: ${err.message}`);
...
async function handler(event, context) {
    try {
        console.log('Starting PDF Document Protection Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const protectionConfig = event.protectionConfig || {
            password: '1234',
            pdfPermission: 'All'
        };
        ...
        console.log(`  Password: ${protectionConfig.password}`);
        console.log(`  Permissions: ${protectionConfig.pdfPermission}`);

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await protectPdfDocument(base64Content, path.basename(inputPdfPath), protectionConfig);
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('PDF document protection completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
            ...
            console.log(`PDF document protection failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        console.log('Starting PDF Document Protection Process (Local Test)');
        ...
        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output protected PDF: ${outputPdfName}`);
        console.log('Protection Configuration:');
        console.log(`  Password: ${protectionConfig.password}`);
        console.log(`  Permissions: ${protectionConfig.pdfPermission}`);
        ...
        if (result.status === 'success') {
            ...
            console.log(`Protected file: ${outputPdfName}`);
            console.log('PDF document has been protected with password');
        ...
        console.error(`PDF document protection failed: ${err.message}`);
        process.exit(1);
    }
}
