...
/**
 * PDF Document Protection using PDF4Me API
 * Protects PDF documents with password and permission restrictions
 * Supports both synchronous and asynchronous processing with retry logic
 */

    // API Configuration - PDF4Me service for protecting PDF documents
    const API_KEY = "Please get the API key from https://dev.pdf4me.com/dashboard/#/api-keys";
const BASE_URL = "https://api.pdf4me.com/";
const API_ENDPOINT = `${BASE_URL}api/v2/Protect`;

// File paths configuration
const PDF_FILE_PATH = "sample.pdf";                    // Path to input PDF file
const OUTPUT_PATH = "sample.protected.pdf";           // Output protected PDF file name
...
/**
 * Main function that orchestrates the PDF document protection process
 * Handles file validation, protection, and result processing
 */
async function protectDocument() {
    console.log("Starting PDF Document Protection Process");
    console.log("=== Protecting PDF Document ===");
    console.log("This adds password protection and permission restrictions");
    ...
    try {
        ...
        console.log(`Protecting: ${PDF_FILE_PATH} → ${OUTPUT_PATH}`);

        // Process the document protection
        const result = await processDocumentProtection();

        // Handle the result
        await handleProtectionResult(result);
...
function readAndEncodePdf(filePath) {
    ...
    if (!fs.existsSync(filePath)) {
        console.log(`Error: PDF file not found at ${filePath}`);
...
/**
 * Core protection logic - handles the API request and response processing
 * Supports both synchronous (200) and asynchronous (202) responses
 */
async function processDocumentProtection() {
    // Read and encode PDF file to base64
    console.log("Reading and encoding PDF file...");
    const base64Content = readAndEncodePdf(PDF_FILE_PATH);
    console.log("PDF file successfully encoded to base64");

    // Prepare the protection payload with all required parameters
    const payload = {
    ...
    console.log(`Protection password: ${payload.password}`);
    console.log(`PDF permissions: ${payload.pdfPermission}`);
    ...
    try {
        ...
        if (response.status === 200) {
            console.log("Success! PDF protected successfully!");
            
            // API returns binary protected PDF content directly for 200 response
...
/**
 * Handles the protection result and saves the protected PDF file
 * Supports both binary protected PDF data and base64 encoded responses
 */
async function handleProtectionResult(result) {
    try {
        // Handle synchronous response (status 200) - Following Java logic
        if (result.binaryContent && result.status === "success") {
            console.log("Processing binary protected PDF response directly...");
            
            // Save the binary protected PDF content directly
            const buffer = Buffer.from(result.binaryContent);
            fs.writeFileSync(OUTPUT_PATH, buffer);
            
            console.log(`Protected PDF saved successfully: ${OUTPUT_PATH}`);
            console.log(`Output file size: ${buffer.length} bytes`);
            console.log("PDF document has been protected with password: 1234");
            console.log("The protected PDF requires the password to open");
            return;
        }
        ...
        if (result.jobId || result.requestId || result.location) {
            ...
            if (locationUrl) {
                ...
                for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                    ...
                    try {
                        ...
                        if (statusResponse.status === 200) {
                            ...
                            // Save the binary protected PDF content directly from polling response
                ...
                console.log("Timeout: PDF protection did not complete after multiple retries");
        ...
        throw new Error(`Error saving protected PDF: ${error.message}`);
    }
}

// Main execution - Run the protection when script is executed directly
if (require.main === module) {
    protectDocument().catch(error => {
        console.error("Fatal error:", error.message);
        process.exit(1);
    });
}

module.exports = { protectDocument, processDocumentProtection, handleProtectionResult }; 