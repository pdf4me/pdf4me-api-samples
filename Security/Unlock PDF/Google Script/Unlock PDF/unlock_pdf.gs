function unlockPdf() {
  ...
  var url = `${baseUrl}api/v2/Unlock`;
  
  // Set the folder and file name for the input protected PDF
  var folderName = 'PDF4ME input'; // <-- Set your folder name here
  var fileName = 'sample.protected.pdf'; // <-- Set your protected PDF file name here
  ...
  // Set the output folder name for unlocked PDF
  var outputFolderName = 'PDF4ME output'; // <-- Set your output folder name here

  // PDF unlocking configuration
  var unlockPassword = '1234'; // <-- Set the password for the protected PDF here
  ...
  try {
    ...
    Logger.log('Protected PDF file successfully encoded to base64');

    // Encode the PDF file as base64 for API transmission
    var pdfBase64 = Utilities.base64Encode(pdfBlob.getBytes());

    // Prepare the payload for the API request
    // What PDF4me PDF unlocking does:
    // - Removes password protection from PDF documents
    // - Unlocks access to protected content and features
    // - Restores full document functionality
    // - Maintains document integrity and formatting
    var payload = {
      docName: "output.pdf",                               // Name of the file
      docContent: pdfBase64,                               // Base64 encoded PDF content
      password: unlockPassword,                            // Password for the protected PDF
    ...
    // Send the initial PDF unlocking request to the API
    Logger.log('Sending PDF unlocking request to PDF4me API...');
    Logger.log('Processing PDF unlock: ' + fileName);
    Logger.log('Using password: ' + unlockPassword);
    ...
    if (code === 200) {
      // 200 means "Success" - PDF unlocking completed successfully
      Logger.log('Success! PDF unlocking completed!');
      
      // Save the unlocked PDF
      try {
        // Get the binary content from the response
        var unlockedPdfBlob = response.getBlob();
        
        // Process and save unlocked PDF
        processUnlockedPdf(unlockedPdfBlob, outputFolderName, file.getName());
        
      } catch (e) {
        Logger.log('Error processing unlocked PDF: ' + e);
      ...
      // 202 means "Accepted" - API is processing the PDF unlocking asynchronously
      ...
      // Poll the API until PDF unlocking is complete
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        var responseUnlock = UrlFetchApp.fetch(locationUrl, {
          method: 'get',
          headers: headers,
          muteHttpExceptions: true
        });
        
        var pollCode = responseUnlock.getResponseCode();
        ...
        if (pollCode === 200) {
          ...
          try {
            ...
            var unlockedPdfBlob = responseUnlock.getBlob();
            ...
            if (outputFolders.hasNext()) {
              var outputFolder = outputFolders.next();
              var rawBlob = Utilities.newBlob(responseUnlock.getContentText(), 'text/plain', 'raw_response.txt');
              outputFolder.createFile(rawBlob);
              Logger.log('Raw response saved: raw_response.txt');
            }
          }
          ...
          Logger.log('Error during processing: ' + pollCode + ' - ' + responseUnlock.getContentText());
          return;
        }
      }

      // If we reach here, polling timed out
      Logger.log('Timeout: PDF unlocking did not complete after multiple retries');
...
function processUnlockedPdf(unlockedPdfBlob, outputFolderName, fileName) {
  // Process and save unlocked PDF
  try {
    var outputFolders = DriveApp.getFoldersByName(outputFolderName);
    if (!outputFolders.hasNext()) {
    ...
    var outputFileName = baseName + '.unlocked.pdf';
    
    // Set the MIME type for PDF
    unlockedPdfBlob.setContentType('application/pdf');
    
    // Save the unlocked PDF file
    var savedFile = outputFolder.createFile(unlockedPdfBlob);
    savedFile.setName(outputFileName);
    
    Logger.log('Unlocked PDF saved: ' + outputFileName);
    Logger.log('Unlocked file size: ' + savedFile.getSize() + ' bytes');
    Logger.log('PDF unlocking completed successfully!');
    
  } catch (e) {
    Logger.log('Error processing unlocked PDF: ' + e);
    
    // Create error file
    var errorContent = 'PDF Unlocking Error\n' +
                      '===================\n' +
                      'Error occurred on: ' + new Date().toString() + '\n\n' +
                      'Error details: ' + e + '\n';
    
    var errorBlob = Utilities.newBlob(errorContent, 'text/plain', 'unlock_error.txt');
    var outputFolders = DriveApp.getFoldersByName(outputFolderName);
    if (outputFolders.hasNext()) {
      var outputFolder = outputFolders.next();
      outputFolder.createFile(errorBlob);
      Logger.log('Error info saved: unlock_error.txt');
    }
  }
} 