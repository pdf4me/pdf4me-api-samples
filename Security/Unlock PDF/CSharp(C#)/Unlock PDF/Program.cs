...
/// <summary>
/// Main program class for unlocking PDF documents functionality
/// This program demonstrates how to unlock password-protected PDF documents using the PDF4ME API
/// </summary>
public class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        var result = await pdfUnlocker.UnlockPdfAsync();
        
        // Display the result
        if (!string.IsNullOrEmpty(result))
            Console.WriteLine($"Unlocked PDF saved to: {result}");
        else
            Console.WriteLine("PDF unlocking failed.");
    }
}

/// <summary>
/// Class responsible for unlocking password-protected PDF documents using the PDF4ME API
/// </summary>
public class UnlockPdf
{
    ...
    /// <summary>
    /// Path where the unlocked PDF file will be saved
    /// </summary>
    ...
    /// <summary>
    /// Constructor to initialize the PDF unlocker
    /// </summary>
    ...
    /// <summary>
    /// Unlocks password-protected PDF document asynchronously using the PDF4ME API
    /// </summary>
    /// <returns>Path to the unlocked PDF file, or null if processing failed</returns>
    public async Task<string?> UnlockPdfAsync()
    {
        try
        {
        ...
        {
            Console.WriteLine($"Error in UnlockPdfAsync: {ex.Message}");
            return null;
        }
    }

    /// <summary>
    /// Executes the PDF unlock operation asynchronously
    /// </summary>
    /// <param name="payload">API request payload</param>
    /// <returns>Path to the unlocked PDF file, or null if processing failed</returns>
    private async Task<string?> ExecutePdfUnlockAsync(object payload)
    {
        try
        {
            ...
            {
                // Read the unlocked PDF content from the response
                byte[] resultBytes = await response.Content.ReadAsByteArrayAsync();
                
                // Save the unlocked PDF to the output path
            ...
            else if ((int)response.StatusCode == 202)
            {
                ...
                Console.WriteLine("Timeout: PDF unlocking did not complete after multiple retries.");
                return null;
            }
        ...
        {
            Console.WriteLine($"Error in ExecutePdfUnlockAsync: {ex.Message}");
            return null;
        }
    }
}