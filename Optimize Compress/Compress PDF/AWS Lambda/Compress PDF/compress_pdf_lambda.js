//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me PDF Compression and Optimization                          //
...
const URL = `${BASE_URL}api/v2/Optimize`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const OUTPUT_PATH = "compress_PDF_output.pdf";
 ...
 * @param {string} optimizeProfile - Optimization profile for compression
 ...
 */
async function compressPdf(base64Content, filename, optimizeProfile = 'Web') {
    const headers = {
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            ...
            console.log(`Compressed PDF saved successfully: ${outputPdfPath}`);
...
async function handler(event, context) {
    try {
        console.log('Starting PDF Compression and Optimization Process in Lambda');
        const inputPdfPath = event.inputPdfPath || 'sample.pdf';
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const optimizeProfile = event.optimizeProfile || 'Web';

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Output processed PDF: ${outputPdfName}`);
        console.log(`Optimization profile: ${optimizeProfile}`);
        console.log('Operation: Compress and optimize PDF');

        const base64Content = readAndEncodePdf(inputPdfPath);
        const apiResponse = await compressPdf(base64Content, path.basename(inputPdfPath), optimizeProfile);
        ...
        if (result.status === 'success') {
            ...
            console.log(`PDF compression failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
...
async function main() {
    try {
        ...
        if (result.status === 'success') {
            ...
            console.log(`Optimization profile used: ${optimizeProfile}`);
        } else {
            console.log(`PDF compression failed: ${result.error}`);
