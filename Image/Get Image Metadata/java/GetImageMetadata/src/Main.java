...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String inputImagePath = "Image/Get Image Metadata/java/GetImageMetadata/sample.png";
        String outputJsonPath = "Image/Get Image Metadata/java/GetImageMetadata/Image_metadata_output.json";
        getImageMetadata(inputImagePath, outputJsonPath);
    }

    public static void getImageMetadata(String inputImagePath, String outputJsonPath) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                ...
                .build();
            System.out.println("Sending image metadata extraction request to PDF4me API...");
            HttpResponse<byte[]> response = httpClient.send(request, HttpResponse.BodyHandlers.ofByteArray());
            System.out.println("Response Status Code: " + response.statusCode());
            if (response.statusCode() == 200) {
                Files.write(Paths.get(outputJsonPath), response.body());
                System.out.println("✓ Success! Image metadata extraction completed!");
                System.out.println("File saved: " + outputJsonPath);
                return;
                ...
                for (int attempt = 0; attempt < maxRetries; attempt++) {
                    ...
                    if (pollResponse.statusCode() == 200) {
                        Files.write(Paths.get(outputJsonPath), pollResponse.body());
                        System.out.println("✓ Success! Image metadata extraction completed!");
                        System.out.println("File saved: " + outputJsonPath);
                        return;
