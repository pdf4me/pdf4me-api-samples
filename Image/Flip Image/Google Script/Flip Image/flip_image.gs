function flipImage() {
  ...
  try {
    ...
    // Prepare API request
    var url = `${baseUrl}api/v2/FlipImage`;
    var headers = {
    ...
    if (responseCode === 200) {
      ...
      var flippedImageBase64 = responseData.imageContent;
      
      // Save output image
      var outputFolder = DriveApp.getFoldersByName(outputFolderName).next();
      var outputImageBlob = Utilities.newBlob(Utilities.base64Decode(flippedImageBase64), 'image/jpeg', outputFileName);
      outputFolder.createFile(outputImageBlob);
      
      Logger.log('Image successfully flipped');
        ...
        while (attempt < maxAttempts && !completed) {
          ...
          if (statusCode === 200) {
            ...
            if (statusData.status === 'completed') {
              ...
              var flippedImageBase64 = resultData.imageContent;
