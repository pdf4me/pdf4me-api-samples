function removeExifTagsFromImage() {
  // Configuration
  ...
  try {
    ...
    // Prepare API request
    var url = `${baseUrl}api/v2/RemoveExifTagsFromImage`;
    var headers = {
    ...
    if (responseCode === 200) {
      ...
      var cleanedImageBase64 = responseData.imageContent;
      
      // Save output image
      var outputFolder = DriveApp.getFoldersByName(outputFolderName).next();
      var outputImageBlob = Utilities.newBlob(Utilities.base64Decode(cleanedImageBase64), 'image/jpeg', outputFileName);
      outputFolder.createFile(outputImageBlob);
      
      Logger.log('EXIF tags successfully removed from image');
        ...
        while (attempt < maxAttempts && !completed) {
          ...
          if (statusCode === 200) {
            ...
            if (statusData.status === 'completed') {
              ...
              var cleanedImageBase64 = resultData.imageContent;
