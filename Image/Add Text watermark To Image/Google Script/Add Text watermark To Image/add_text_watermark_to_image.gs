function addTextWatermarkToImage() {
  ...
  var outputFileName = 'sample_text_watermarked.jpg';
  var outputFolderName = 'PDF4ME output'; // <-- Set your output folder name here

  // Text watermark configuration
  var watermarkText = 'CONFIDENTIAL'; // <-- Set your watermark text here
  ...
  try {
    ...
    var payload = {
        ...
        watermarkText: watermarkText,     // Text to use as watermark
    ...
    // Send the image to the API for adding text watermark
    Logger.log('Sending add text watermark request...');
    Logger.log('Input image file: ' + fileName);
    Logger.log('Watermark text: ' + watermarkText);
    Logger.log('Position: ' + position);
    Logger.log('Opacity: ' + opacity);
    Logger.log('Font size: ' + fontSize);
    Logger.log('Font color: ' + fontColor);
    Logger.log('Font family: ' + fontFamily);
    Logger.log('Output image file: ' + outputFileName);
    Logger.log('Adding text watermark...');
    ...
    if (code === 200) {
      Logger.log('Add text watermark completed successfully!');
      ...
      Logger.log('Text watermarked image saved to folder: ' + outputFolderName);
      Logger.log('Text watermark has been successfully added to the image');
      return;
    }
    // If processing is asynchronous, poll for the result
      ...
      Logger.log('Timeout: Add text watermark did not complete after multiple retries.');
    } 
