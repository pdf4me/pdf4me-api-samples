// =============================================================================
// AWS Lambda Handler for PDF4me Add Text Watermark To Image                       //
// =============================================================================
...
const API_URL = 'https://api.pdf4me.com/api/v2/AddTextWatermarkToImage';
...
exports.handler = async (event, context) => {
    console.log('=== Add Text Watermark To Image Lambda Function ===');
    ...
    try {
        ...
        if (event.httpMethod) {
            ...
            inputData = {
                ...
                watermarkFontSize: process.env.WATERMARK_FONT_SIZE || 24,
        ...
        if (!inputData.watermarkText) {
            throw new Error('Missing watermarkText in input data');
        }

        // Add text watermark to image
        const result = await addTextWatermarkToImage(inputData);

        // Save result to S3 if output bucket is configured
        if (OUTPUT_BUCKET && result) {
            const outputKey = `text-watermarked-image-${Date.now()}.png`;
            await s3.putObject({
        ...
        return {
            ...
            body: JSON.stringify({
                ...
                outputLocation: OUTPUT_BUCKET ? `s3://${OUTPUT_BUCKET}/text-watermarked-image-${Date.now()}.png` : null
            })
        };
 ...
 */
async function addTextWatermarkToImage(inputData) {
    try {
        console.log('Sending request to PDF4me API...');
        console.log(`Adding text watermark to: ${inputData.imageName}`);
        console.log(`Watermark text: ${inputData.watermarkText}`);
        ...
        if (response.status === 200) {
            console.log('Text watermark added immediately');
            return handleWatermarkResult(response.data);
        } else if (response.status === 202) {
            console.log('Text watermark operation started asynchronously, polling for completion...');
...
/**
 * Poll for completion of asynchronous text watermark operation
...
async function pollForCompletion(locationUrl) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (response.status === 200) {
                console.log('Text watermark operation completed successfully');
                return handleWatermarkResult(response.data);
            } else if (response.status === 202) {
            ...
            if (attempt === maxAttempts) {
                throw new Error(`Text watermark operation timed out after ${maxAttempts} attempts`);
            }
        }
    }

    throw new Error(`Text watermark operation timed out after ${maxAttempts} polling attempts`);
}

/**
 * Handle the text watermark result from API response
 * @param {Object} responseData - API response data
 * @returns {string} Watermarked image content as base64
 */
function handleWatermarkResult(responseData) {
    try {
        console.log('Processing text watermark result...');

        // Check if response contains image data
        if (responseData.imageData) {
            console.log('Successfully received text watermarked image data');
            return responseData.imageData;
        } else if (responseData.data && responseData.data.imageData) {
            console.log('Successfully received text watermarked image data from nested structure');
        ...
        console.error('Error handling text watermark result:', error.message);
        throw new Error('Failed to process text watermark result');
    }
} 