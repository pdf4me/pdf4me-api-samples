function replaceTextWithImage() {
  ...
  var position = 'center'; // Position: 'top-left', 'top-center', 'top-right', 'center-left', 'center', 'center-right', 'bottom-left', 'bottom-center', 'bottom-right'
  ...
  try {
    ...
    // Get replacement image file
    var replacementFile = inputFolder.getFilesByName(replacementImageFileName).next();
    var replacementBlob = replacementFile.getBlob();
    var replacementBase64 = Utilities.base64Encode(replacementBlob.getBytes());
    
    // Prepare API request
    var url = `${baseUrl}api/v2/ReplaceTextWithImage`;
    var headers = {
    ...
    var payload = {
      ...
      replacementImageContent: replacementBase64,
    ...
    if (responseCode === 200) {
      ...
      var modifiedImageBase64 = responseData.imageContent;
      
      // Save output image
      var outputFolder = DriveApp.getFoldersByName(outputFolderName).next();
      var outputImageBlob = Utilities.newBlob(Utilities.base64Decode(modifiedImageBase64), 'image/jpeg', outputFileName);
      outputFolder.createFile(outputImageBlob);
      
      Logger.log('Text successfully replaced with image');
        ...
        while (attempt < maxAttempts && !completed) {
          ...
          if (statusCode === 200) {
            ...
            if (statusData.status === 'completed') {
              ...
              var modifiedImageBase64 = resultData.imageContent;
