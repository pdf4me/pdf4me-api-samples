function rotateImageByExifData() {
  // Configuration
  ...
  try {
    ...
    // Prepare API request
    var url = `${baseUrl}api/v2/RotateImageByExifData`;
    var headers = {
    ...
    if (responseCode === 200) {
      ...
      var correctedImageBase64 = responseData.imageContent;
      
      // Save output image
      var outputFolder = DriveApp.getFoldersByName(outputFolderName).next();
      var outputImageBlob = Utilities.newBlob(Utilities.base64Decode(correctedImageBase64), 'image/jpeg', outputFileName);
      outputFolder.createFile(outputImageBlob);
      
      Logger.log('Image successfully rotated based on EXIF data');
        ...
        while (attempt < maxAttempts && !completed) {
          ...
          if (statusCode === 200) {
            ...
            if (statusData.status === 'completed') {
              ...
              var correctedImageBase64 = resultData.imageContent;
