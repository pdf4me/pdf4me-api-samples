...
public class Main {
    ...
    public static void main(String[] args) {
        System.out.println("Rotating image by EXIF data...");
        String inputImagePath = "Image/Rotate Image By Exif Data/java/RotateImageByExifData/pdf4me.png";
        String outputImagePath = "Image/Rotate Image By Exif Data/java/RotateImageByExifData/Rotate_image_by_exif_data_output.png";
        rotateImageByExifData(inputImagePath, outputImagePath);
    }

    public static void rotateImageByExifData(String inputImagePath, String outputImagePath) {
        try {
            ...
            String payload = String.format("{\"docContent\":\"%s\",\"docName\":\"%s\",\"isAsync\":true}", imageBase64, docName);
            HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + "api/v2/RotateImageByExifData"))
                .header("Content-Type", "application/json")
                .header("Authorization", "Basic " + API_KEY)
                .POST(HttpRequest.BodyPublishers.ofString(payload))
                .build();
            System.out.println("Sending image rotation by EXIF data request to PDF4me API...");
            HttpResponse<byte[]> response = httpClient.send(request, HttpResponse.BodyHandlers.ofByteArray());
            System.out.println("Response Status Code: " + response.statusCode());
            if (response.statusCode() == 200) {
                Files.write(Paths.get(outputImagePath), response.body());
                System.out.println("✓ Success! Image rotation by EXIF data completed!");
