 ...
 * Crops images with specified dimensions and position
 * Supports both synchronous and asynchronous processing with retry logic
 */

// API Configuration - PDF4Me service for cropping images
const API_KEY = "get the API key from https://dev.pdf4me.com/dashboard/#/api-keys";
const BASE_URL = "https://api.pdf4me.com/";
const API_ENDPOINT = `${BASE_URL}api/v2/CropImage`;

// File paths configuration
const INPUT_IMAGE_PATH = "sample.jpg";                             // Input image file to crop
const OUTPUT_IMAGE_PATH = "Crop_image_output.jpg";                 // Output cropped image file
...
async function main() {
    ...
    try {
        const result = await cropImage(INPUT_IMAGE_PATH);
        ...
        if (result) {
            ...
            console.log("✅ Image has been cropped and is ready for use");
        } else {
...
/**
 * Crops an image using PDF4Me service
 * @param {string} inputImagePath - Path to the input image file
 * @returns {Promise<string|null>} - Path to the cropped image file, or null if failed
 */
async function cropImage(inputImagePath) {
    try {
        ...
        const payload = {
            ...
            cropX: 100,                                            // X coordinate of crop start
        ...
        console.log(`📐 Cropping area: 400x300 pixels starting at (100,100)`);
        
        // Execute the image crop operation
        return await executeImageCrop(payload, OUTPUT_IMAGE_PATH);
...
/**
 * Executes the image crop operation
 * @param {Object} payload - API request payload
 * @param {string} outputImagePath - Path where to save the cropped image
 * @returns {Promise<string|null>} - Path to the cropped image file, or null if failed
 */
async function executeImageCrop(payload, outputImagePath) {
    try {
        ...
        // Handle asynchronous processing response (202 Accepted)
            ...
            console.log("❌ Timeout: Image cropping did not complete after multiple retries.");
            return null;
        }
        ...
        console.error("❌ Error in executeImageCrop:", error.message);
        return null;
    }
}
...
module.exports = { cropImage, executeImageCrop }; 