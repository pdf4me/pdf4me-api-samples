...
public class Main {
    ...
    public static void main(String[] args) {
        System.out.println("=== Splitting PDF by Barcode ===");
        try {
            String inputPdfPath = "sample.pdf";
            String outputDir = inputPdfPath.replace(".pdf", "_barcode_split_output");
            splitPdfByBarcode(inputPdfPath, outputDir,
                "hello", // barcodeString
    ...
    public static void splitPdfByBarcode(String inputPdfPath, String outputFolder, String barcodeString, String barcodeFilter, String barcodeType, String splitBarcodePage, boolean combinePagesWithSameConsecutiveBarcodes, String pdfRenderDpi) {
        try {
    ...
    private static void executeSplitByBarcode(Map<String, Object> payload, Path outputDir) {
        try {
            ...
            if (response.statusCode() == 200) {
                saveZipResult(response.body(), outputDir, "barcode_split_result.zip");
                ...
                for (int attempt = 0; attempt < maxRetries; attempt++) {
                    ...
                    if (pollResponse.statusCode() == 200) {
                        saveZipResult(pollResponse.body(), outputDir, "barcode_split_result.zip");
                        return;
                ...
                System.out.println("Timeout: Split by barcode did not complete after multiple retries.");
            } else {
                System.out.println("Initial request failed: " + response.statusCode());
                System.out.println(new String(response.body()));
            }
        } catch (Exception ex) {
            System.err.println("Error in executeSplitByBarcode: " + ex.getMessage());
        }
    }
    ...
    private static void saveZipResult(byte[] responseBody, Path outputDir, String zipFileName) {
        try {
            ...
            System.out.println("Split by barcode zip saved: " + outPath.toAbsolutePath());
        } catch (Exception ex) {
