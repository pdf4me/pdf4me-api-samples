function splitPdfBySwissQr() {
  /**
   * Split PDF document by Swiss QR codes using PDF4Me API
   * Process: Read PDF → Encode to base64 → Send API request → Poll for completion → Save split PDFs
   * This action separates PDF documents based on Swiss QR codes for automated invoice processing
   */
  ...
  var url = baseUrl + "api/v2/SplitPdfBySwissQR";
  ...
  try {
    console.log("Splitting PDF by Swiss QR codes...");
    ...
    try {
      ...
      console.log('PDF file read successfully: ' + pdfBlob.getBytes().length + ' bytes (Base64 length: ' + pdfContent.length + ' characters)');
    } catch (e) {
      console.log('Error reading PDF file: ' + e.toString());
      return;
    }

    // Request payload with PDF content and Swiss QR split parameters
    var payload = {
      "docContent": pdfContent,                                  // Base64 encoded PDF content
      "docName": file.getName(),                                 // Source PDF file name with extension
      "splitQRPage": "after",                                    // Split position: after, before
    ...
    // Make the API request to split PDF by Swiss QR
    try {
      var response = UrlFetchApp.fetch(url, options);
      var code = response.getResponseCode();
      
      // Log detailed response information for debugging 
      console.log('Response Status Code: ' + code + ' (' + response.getHeaders()['Status'] + ')');
      console.log('Response Headers:');
      var allHeaders = response.getAllHeaders();
      for (var headerName in allHeaders) {
        console.log('  ' + headerName + ': ' + allHeaders[headerName]);
      }
    ...
    if (code === 200) {
      ...
      try {
        ...
        if (Array.isArray(splitData)) {
          ...
          console.log("Response is not a list - checking for splitedDocuments structure...");
      ...
      // Poll the API until PDF Swiss QR splitting is complete
