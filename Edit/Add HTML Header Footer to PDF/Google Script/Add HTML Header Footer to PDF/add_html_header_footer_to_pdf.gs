function addHtmlHeaderFooterToPdf() {
  ...
  var url = `${baseUrl}api/v2/AddHtmlHeaderFooter`;
  ...
  // Set the output file name for the PDF with HTML header/footer
  var outputFileName = 'Add_header_footer_to_PDF_output.pdf';
  ...
  try {
    ...
    // What HTML header/footer addition does:
    // - Adds HTML content as headers, footers, or both to PDF documents
    // - Supports custom HTML styling, fonts, colors, and positioning
    // - Allows precise control over margins and page selection
    // - Can skip first page or apply to specific page ranges
    var payload = {
    ...
    // Send the initial HTML header/footer addition request to the API
    Logger.log('Sending HTML header/footer request to PDF4Me API...');
    Logger.log('Adding HTML header/footer to: ' + fileName + ' → ' + outputFileName);
    Logger.log('Location: ' + payload.location + ' | Pages: ' + (payload.pages || 'All pages'));
    ...
    if (code === 200) {
      // 200 means "Success" - HTML header/footer addition completed successfully
      Logger.log(' Success! HTML header/footer addition completed!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var headerFooterPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(headerFooterPdfBlob);
        Logger.log('PDF with HTML header/footer saved to: ' + outputFileName);
        Logger.log('HTML header/footer successfully added to PDF');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the HTML header/footer addition asynchronously
      ...
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          // Validate and save the PDF with HTML header/footer
          ...
          if (responseText.startsWith('%PDF') || responseConversion.getBlob().getBytes().length > 1000) {
            var headerFooterPdfBlob = responseConversion.getBlob().setName(outputFileName);
