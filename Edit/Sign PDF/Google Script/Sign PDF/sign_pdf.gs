function signPdf() {
  ...
  var url = `${baseUrl}api/v2/SignPdf`;
  ...
  var signatureFileName = 'dev.jpg'; // <-- Set your signature image file name here

  //         ===  Set the file ID for the input files ===
  // var pdfFileId = '1Py5fWBBgk0gmRRfZnrSyYUOOZEsUkk8q'; // 
  // var signatureFileId = '1Qz9xRCCll1mNnOoPpQqRrSsTtUuVvWwXxYyZz'; // 
  ...
  // Set the output file name for the signed PDF
  var outputFileName = 'Add_sign_to_PDF_output.pdf';
  ...
  try {
    ...
    // Get the signature image file by name within the folder
    var signatureFiles = folder.getFilesByName(signatureFileName);
    if (!signatureFiles.hasNext()) {
      Logger.log('Signature image file not found in folder: ' + signatureFileName);
      return;
    }
    // Get the first signature file found
    var signatureFile = signatureFiles.next();

    // === Folder structure file input END ===

    // === File ID as input START ===
    // var pdfFile = DriveApp.getFileById(pdfFileId);
    // var signatureFile = DriveApp.getFileById(signatureFileId);
    ...
    // Get the signature image file as a blob (binary data)
    var signatureBlob = signatureFile.getBlob();
    Logger.log('Signature file name: ' + signatureFile.getName());
    ...
    // Encode the signature image file as base64 for API transmission
    var signatureBase64 = Utilities.base64Encode(signatureBlob.getBytes());

    // Prepare the payload for the API request
    // What PDF signing does:
    // - Adds signature images to PDF documents with precise positioning
    // - Supports various signature formats (JPG, PNG, etc.)
    // - Allows control over signature size, position, and appearance
    // - Useful for document authentication, approval workflows, and legal compliance
    var payload = {
      docContent: pdfBase64,                        // Base64 encoded PDF document content
      docName: 'output.pdf',                        // Output PDF file name
      imageFile: signatureBase64,                   // Base64 encoded signature image content
      imageName: signatureFile.getName(),           // Signature image file name with extension
    ...
    // Send the initial signature request to the API
    Logger.log('Sending signature request to PDF4me API...');
    Logger.log('Adding signature to: ' + pdfFileName + ' → ' + outputFileName);
    ...
    if (code === 200) {
      // 200 means "Success" - signature addition completed successfully
      Logger.log(' Success! Signature addition completed!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var signedPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(signedPdfBlob);
        Logger.log('Signed PDF saved to: ' + outputFileName);
        Logger.log('Signature successfully added to PDF');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the signature addition asynchronously
      ...
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          // Validate and save the signed PDF
          ...
          if (responseText.startsWith('%PDF') || responseConversion.getBlob().getBytes().length > 1000) {
            var signedPdfBlob = responseConversion.getBlob().setName(outputFileName);
