function addPageNumberToPdf() {
  ...
  var url = `${baseUrl}api/v2/AddPageNumber`;
  ...
  // Set the output file name for the PDF with page numbers
  var outputFileName = 'Add_page_number_to_PDF_output.pdf';
  ...
  try {
    ...
    // What page number addition does:
    // - Adds customizable page numbers to PDF documents
    // - Supports various page number formats and styles
    // - Allows precise positioning and alignment control
    // - Can skip first page or apply to all pages
    // - Useful for professional document formatting and navigation
    var payload = {
    ...
    // Send the initial page number addition request to the API
    Logger.log('Sending page number request to PDF4Me API...');
    Logger.log('Adding page numbers to: ' + fileName + ' → ' + outputFileName);
    ...
    if (code === 200) {
      // 200 means "Success" - page number addition completed successfully
      Logger.log(' Success! Page number addition completed!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var pageNumberPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(pageNumberPdfBlob);
        Logger.log('PDF with page numbers saved to: ' + outputFileName);
        Logger.log('Page numbers successfully added to PDF');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the page number addition asynchronously
      ...
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          // Validate and save the PDF with page numbers
          ...
          if (responseText.startsWith('%PDF') || responseConversion.getBlob().getBytes().length > 1000) {
            var pageNumberPdfBlob = responseConversion.getBlob().setName(outputFileName);
