//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Add Image Stamp to PDF                                      //
// Get Your API Key: https://dev.pdf4me.com/dashboard/#/api-keys                            //
// API Documentation: https://developer.pdf4me.com/swagger/index.html?url=/swagger/v2/swagger.json //
//                                                                                          //
// Note: Replace placeholder values in the code with your API Key                           //
//                                                                                          //
//*******************************************************************************************//

const fs = require('fs');
const path = require('path');
const axios = require('axios');

// API Configuration - PDF4me service for adding image stamps to PDF documents
const API_KEY = process.env.PDF4ME_API_KEY || 'get the API key from https://dev.pdf4me.com/dashboard/#/api-keys';

// API endpoint for adding image stamps to PDF documents
const BASE_URL = 'https://api.pdf4me.com/';
const URL = `${BASE_URL}api/v2/ImageStamp`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const IMAGE_FILE_PATH = "pdf4me.png";
const OUTPUT_PATH = "Add_image_stamp_to_PDF_output.pdf";

// Processing configuration
const MAX_RETRIES = 10;
const RETRY_DELAY = 10; // seconds

/**
 * Read file and convert it to base64 encoding
 * @param {string} filePath - Path to the file to be processed
 * @returns {string} Base64 encoded content of the file
 * @throws {Error} If the specified file doesn't exist or reading fails
 */
function readAndEncodeFile(filePath) {
    if (!fs.existsSync(filePath)) {
        const errMsg = `Error: File not found at ${filePath}`;
        console.error(errMsg);
        throw new Error(`File not found: ${filePath}`);
    }
    try {
        const fileContent = fs.readFileSync(filePath);
        const base64Content = Buffer.from(fileContent).toString('base64');
        console.log(`File read successfully: ${fileContent.length} bytes`);
        return base64Content;
    } catch (err) {
        const errMsg = `Error reading file: ${err.message}`;
        console.error(errMsg);
        throw err;
    }
}

/**
 * Send PDF and image to PDF4me API for image stamp addition
 * @param {string} pdfBase64 - Base64 encoded PDF content
 * @param {string} imageBase64 - Base64 encoded image content
 * @param {string} imageFilename - Name of the image file
 * @param {Object} stampConfig - Image stamp configuration
 * @returns {Promise<Object>} API response containing the processed file or processing status
 * @throws {Error} For API request errors
 */
async function addImageStamp(pdfBase64, imageBase64, imageFilename, stampConfig = {}) {
    const headers = {
        'Content-Type': 'application/json',
        'Authorization': `Basic ${API_KEY}`
    };

    const payload = {
        docContent: pdfBase64,                    // Base64 encoded PDF document content
        docName: "output.pdf",                    // Output PDF file name
        alignX: stampConfig.alignX || "Center",   // Horizontal alignment: "Left", "Center", "Right"
        alignY: stampConfig.alignY || "Middle",   // Vertical alignment: "Top", "Middle", "Bottom"
        imageFile: imageBase64,                   // Base64 encoded image content
        imageName: imageFilename,                 // Image file name with extension
        pages: stampConfig.pages || "",           // Page options: "", "1", "1,3,5", "2-5", "1,3,7-10", "2-"
        heightInMM: stampConfig.heightInMM || "30", // Image height in millimeters (10-200)
        widthInMM: stampConfig.widthInMM || "30",  // Image width in millimeters (10-200)
        heightInPx: stampConfig.heightInPx || "85", // Image height in pixels (20-600)
        widthInPx: stampConfig.widthInPx || "85",   // Image width in pixels (20-600)
        marginXInMM: stampConfig.marginXInMM || "10", // Horizontal margin in millimeters (0-100)
        marginYInMM: stampConfig.marginYInMM || "10", // Vertical margin in millimeters (0-100)
        marginXInPx: stampConfig.marginXInPx || "28", // Horizontal margin in pixels (0-300)
        marginYInPx: stampConfig.marginYInPx || "28", // Vertical margin in pixels (0-300)
        opacity: stampConfig.opacity || 50,       // Opacity (0-100): 0=invisible, 100=fully opaque
        isBackground: stampConfig.isBackground !== undefined ? stampConfig.isBackground : true, // Place stamp in background/foreground (true/false)
        showOnlyInPrint: stampConfig.showOnlyInPrint || false, // Show in view and print (true/false)
        async: true                               // Enable asynchronous processing
    };

    console.log('Sending image stamp request to PDF4me API...');

    try {
        const response = await axios.post(URL, payload, {
            headers,
            timeout: 30000,
            responseType: 'arraybuffer', // Ensure response is treated as binary
            validateStatus: () => true
        });

        console.log(`Response Status Code: ${response.status} (${response.statusText})`);
        console.log('Response Headers:');
        for (const [headerName, headerValue] of Object.entries(response.headers)) {
            console.log(`  ${headerName}: ${headerValue}`);
        }

        if (response.status === 200) {
            console.log('Success! Image stamp addition completed successfully!');
            return { binaryContent: response.data, status: 'success' };
        } else if (response.status === 202) {
            console.log('Request accepted. Processing asynchronously...');
            const locationUrl = response.headers['location'];
            console.log(`Location URL: ${locationUrl || 'NOT FOUND'}`);
            return { jobId: null, location: locationUrl, status: 'processing' };
        } else {
            const errorData = Buffer.from(response.data).toString('utf8');
            console.log(`Error: ${response.status} - ${errorData}`);
            throw new Error(`API Error: ${response.status} - ${errorData}`);
        }
    } catch (err) {
        if (err.code === 'ECONNABORTED') {
            console.error('Error: Request timeout. The API took too long to respond.');
        } else if (err.response) {
            const errorData = Buffer.from(err.response.data).toString('utf8');
            console.error(`API Request Error: ${err.response.status} - ${errorData}`);
        } else {
            console.error(`API Request Error: ${err.message}`);
        }
        throw err;
    }
}

/**
 * Handle API response and save the PDF file with image stamp
 * @param {Object} apiResponse - Response from the PDF4me API
 * @param {string} outputPdfPath - Path where to save the processed PDF
 * @returns {Promise<Object>} Object containing output path and status
 */
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            console.log('Processing binary PDF response directly...');
            fs.writeFileSync(outputPdfPath, Buffer.from(apiResponse.binaryContent));
            console.log(`PDF with image stamp saved successfully: ${outputPdfPath}`);
            console.log(`Output file size: ${apiResponse.binaryContent.length} bytes`);
            return { outputPath: outputPdfPath, status: 'success', fileSize: apiResponse.binaryContent.length };
        } else if (apiResponse.jobId || apiResponse.requestId || apiResponse.location) {
            console.log('Handling asynchronous processing...');
            const locationUrl = apiResponse.location;
            if (locationUrl) {
                const headers = {
                    'Content-Type': 'application/json',
                    'Authorization': `Basic ${API_KEY}`
                };
                for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                    console.log(`Checking job status... (Attempt ${attempt + 1}/${MAX_RETRIES})`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY * 1000));
                    try {
                        const statusResponse = await axios.get(locationUrl, {
                            headers,
                            responseType: 'arraybuffer', // Expect binary response
                            validateStatus: () => true
                        });
                        console.log(`Poll response status: ${statusResponse.status} (${statusResponse.statusText})`);
                        if (statusResponse.status === 200) {
                            console.log('Processing completed!');
                            fs.writeFileSync(outputPdfPath, Buffer.from(statusResponse.data));
                            console.log(`PDF with image stamp saved successfully: ${outputPdfPath}`);
                            console.log(`Output file size: ${statusResponse.data.length} bytes`);
                            return { outputPath: outputPdfPath, status: 'success', fileSize: statusResponse.data.length };
                        } else if (statusResponse.status === 202) {
                            console.log('Still processing...');
                            continue;
                        } else {
                            const errorData = Buffer.from(statusResponse.data).toString('utf8');
                            console.error(`Error during polling: ${statusResponse.status} - ${errorData}`);
                            return { status: 'error', error: `Polling failed: ${statusResponse.status} - ${errorData}` };
                        }
                    } catch (err) {
                        console.error(`Error polling status: ${err.message}`);
                        continue;
                    }
                }
                console.log('Timeout: Processing did not complete after multiple retries');
                return { status: 'timeout', error: 'Processing timeout' };
            } else {
                console.log('No polling URL available for async job');
                return { status: 'no-polling-url', error: 'No polling URL available' };
            }
        } else {
            console.error('Error: Invalid API response format');
            return { status: 'error', error: 'Invalid API response format' };
        }
    } catch (err) {
        console.error(`Error saving processed PDF: ${err.message}`);
        return { status: 'error', error: err.message };
    }
}

/**
 * AWS Lambda handler function
 * @param {Object} event - Lambda event object
 * @param {Object} context - Lambda context object
 * @returns {Object} Lambda response object
 */
async function handler(event, context) {
    try {
        console.log('Starting Add Image Stamp to PDF Process in Lambda');
        const inputPdfPath = event.inputPdfPath || INPUT_PDF_PATH;
        const imageFilePath = event.imageFilePath || IMAGE_FILE_PATH;
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;
        const stampConfig = event.stampConfig || {};

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Image file: ${imageFilePath}`);
        console.log(`Output PDF with image stamp: ${outputPdfName}`);
        console.log(`Stamp configuration:`, stampConfig);
        console.log('Operation: Add image stamp to PDF');

        const pdfBase64 = readAndEncodeFile(inputPdfPath);
        const imageBase64 = readAndEncodeFile(imageFilePath);
        const apiResponse = await addImageStamp(
            pdfBase64, 
            imageBase64, 
            path.basename(imageFilePath),
            stampConfig
        );
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('Image stamp addition completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin': '*'
                },
                body: JSON.stringify({
                    success: true,
                    message: 'Image stamp addition completed successfully',
                    outputFileName: outputPdfName,
                    fileSize: result.fileSize,
                    inputPdf: inputPdfPath,
                    imageFile: imageFilePath,
                    stampConfig: stampConfig
                })
            };
        } else {
            console.log(`Image stamp addition failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin': '*'
                },
                body: JSON.stringify({
                    success: false,
                    message: 'Image stamp addition failed',
                    error: result.error
                })
            };
        }
    } catch (err) {
        console.error(`Lambda execution failed: ${err.message}`);
        return {
            statusCode: 500,
            headers: {
                'Content-Type': 'application/json',
                'Access-Control-Allow-Origin': '*'
            },
            body: JSON.stringify({
                success: false,
                message: 'Lambda execution failed',
                error: err.message
            })
        };
    }
}

// For local testing (not used in Lambda)
async function main() {
    try {
        console.log('Starting Add Image Stamp to PDF Process (Local Test)');
        console.log('=== Adding Image Stamp to PDF Document ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const imageFilePath = IMAGE_FILE_PATH;
        const outputPdfName = OUTPUT_PATH;
        const stampConfig = {
            alignX: "Center",
            alignY: "Middle",
            heightInMM: "30",
            widthInMM: "30",
            heightInPx: "85",
            widthInPx: "85",
            marginXInMM: "10",
            marginYInMM: "10",
            marginXInPx: "28",
            marginYInPx: "28",
            opacity: 50,
            isBackground: true,
            showOnlyInPrint: false
        };

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Image file: ${imageFilePath}`);
        console.log(`Output PDF with image stamp: ${outputPdfName}`);
        console.log(`Stamp configuration:`, stampConfig);
        console.log('Operation: Add image stamp to PDF');

        const pdfBase64 = readAndEncodeFile(inputPdfPath);
        const imageBase64 = readAndEncodeFile(imageFilePath);
        const apiResponse = await addImageStamp(
            pdfBase64, 
            imageBase64, 
            path.basename(imageFilePath),
            stampConfig
        );
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('Image stamp addition completed successfully!');
            console.log(`Input PDF: ${inputPdfPath}`);
            console.log(`Image file: ${imageFilePath}`);
            console.log(`Output file: ${outputPdfName}`);
            console.log(`Stamp configuration used:`, stampConfig);
        } else {
            console.log(`Image stamp addition failed: ${result.error}`);
            console.log('Please check your input files and API configuration');
        }
    } catch (err) {
        console.error(`Image stamp addition failed: ${err.message}`);
        process.exit(1);
    }
}

if (require.main === module) {
    main().catch(console.error);
}

module.exports = { 
    readAndEncodeFile, 
    addImageStamp, 
    handleAsyncResponse, 
    main,
    handler 
}; 