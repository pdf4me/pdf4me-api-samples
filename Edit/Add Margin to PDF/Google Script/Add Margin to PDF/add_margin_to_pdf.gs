function addMarginToPdf() {
  ...
  var url = `${baseUrl}api/v2/AddMargin`;
  ...
  // Set the output file name for the PDF with margins
  var outputFileName = 'Add_margin_to_PDF_output.pdf';
  ...
  try {
    ...
    // What margin addition does:
    // - Adds custom margins to any side of PDF documents
    // - Changes the page size accordingly to accommodate margins
    // - Supports precise margin control in millimeters
    // - Can be applied to all pages in the document
    // - Useful for printing, binding, and document formatting
    var payload = {
    ...
    // Send the initial margin addition request to the API
    Logger.log('Sending margin addition request to PDF4Me API...');
    Logger.log('Adding margins to: ' + fileName + ' → ' + outputFileName);
    Logger.log('Margins: Left=' + payload.marginLeft + 'mm, Right=' + payload.marginRight + 'mm, Top=' + payload.marginTop + 'mm, Bottom=' + payload.marginBottom + 'mm');
    ...
    if (code === 200) {
      // 200 means "Success" - margin addition completed successfully
      Logger.log(' Success! Margin addition completed!');
      ...
      if (contentType.startsWith('application/pdf') || 
        ...
        var marginPdfBlob = responseContent.setName(outputFileName);
        outputFolder.createFile(marginPdfBlob);
        Logger.log('PDF with margins saved to: ' + outputFileName);
        Logger.log('Margins successfully added to PDF');
        return;
      }
      ...
      // 202 means "Accepted" - API is processing the margin addition asynchronously
      ...
      for (var attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (pollCode === 200) {
          ...
          // Validate and save the PDF with margins
          ...
          if (responseText.startsWith('%PDF') || responseConversion.getBlob().getBytes().length > 1000) {
            var marginPdfBlob = responseConversion.getBlob().setName(outputFileName);
