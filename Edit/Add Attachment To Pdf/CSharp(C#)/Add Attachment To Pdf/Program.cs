...
/// <summary>
/// Main program class for adding attachments to PDF functionality
...
public class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        var result = await attachmentAdder.AddAttachmentAsync();
        
        // Display the result
        if (!string.IsNullOrEmpty(result))
            Console.WriteLine($"PDF with attachment saved to: {result}");
        else
            Console.WriteLine("Attachment addition failed.");
    }
}

/// <summary>
/// Class responsible for adding attachments to PDF documents using the PDF4ME API
/// </summary>
public class PdfAttachmentAdder
{
    ...
    /// <summary>
    /// Adds attachment to PDF document asynchronously using the PDF4ME API
    /// </summary>
    ...
    public async Task<string?> AddAttachmentAsync()
    {
        try
        {
            ...
            // Check if sample.txt exists for attachment
            ...
            // Read text file content and convert to base64 for attachment
            string textContent = await File.ReadAllTextAsync(textFilePath);
            byte[] textBytes = Encoding.UTF8.GetBytes(textContent);
            string textBase64 = Convert.ToBase64String(textBytes);

            // Prepare the API request payload according to PDF4ME documentation
            var payload = new
            {
                ...
                attachments = new[]
                {
                    new
                    {
                        // docName - Required: Source file name with extension
                        docName = "sample.txt",
                        // docContent - Required: The content of the attachment file (base64)
            ...
            return await ExecuteAttachmentAdditionAsync(payload);
        }
        catch (Exception ex)
        {
            // Log any errors that occur during the attachment process
            Console.WriteLine($"Error in AddAttachmentAsync: {ex.Message}");
            return null;
        }
    }

    /// <summary>
    /// Executes the attachment addition operation asynchronously
    /// </summary>
    /// <param name="payload">API request payload containing PDF and attachment data</param>
    /// <returns>Path to the modified PDF document, or null if addition failed</returns>
    private async Task<string?> ExecuteAttachmentAdditionAsync(object payload)
    {
        try
        {
            ...
            else if ((int)response.StatusCode == 202)
            {
                ...
                for (int attempt = 0; attempt < maxRetries; attempt++)
                {
                    ...
                    {
                        // Read the completed PDF with attachment from the response
                ...
                // Timeout if attachment addition doesn't complete within retry limit
                Console.WriteLine("Timeout: Attachment addition did not complete after multiple retries.");
                return null;
            }
        ...
        catch (Exception ex)
        {
            ...
            Console.WriteLine($"Error in ExecuteAttachmentAdditionAsync: {ex.Message}");
            return null;
        }
    }
}
