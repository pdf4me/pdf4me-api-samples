//*******************************************************************************************//
//                                                                                          //
// AWS Lambda Handler for PDF4me Add Attachment to PDF                                      //
...
// API Configuration - PDF4me service for adding attachments to PDF documents
...
const URL = `${BASE_URL}api/v2/AddAttachmentToPdf`;

// File paths configuration
const INPUT_PDF_PATH = "sample.pdf";
const ATTACHMENT_FILE_PATH = "sample.txt";
const OUTPUT_PATH = "Add_attachment_to_PDF_output.pdf";
...
/**
 * Send PDF and attachment to PDF4me API for attachment addition
 * @param {string} pdfBase64 - Base64 encoded PDF content
 * @param {string} attachmentBase64 - Base64 encoded attachment content
 * @param {string} pdfFilename - Name of the source PDF file
 ...
 */
async function addAttachmentToPdf(pdfBase64, attachmentBase64, pdfFilename, attachmentFilename) {
    const headers = {
    ...
    const payload = {
        ...
        attachments: [                                  // Array of attachments to add to the PDF
            {
                docName: attachmentFilename,            // Attachment file name with extension
    ...
    try {
        ...
        if (response.status === 200) {
            console.log('Success! Attachment addition completed successfully!');
...
/**
 * Handle API response and save the PDF file with attachment
...
async function handleAsyncResponse(apiResponse, outputPdfPath) {
    try {
        if (apiResponse.binaryContent && apiResponse.status === 'success') {
            ...
            console.log(`PDF with attachment saved successfully: ${outputPdfPath}`);
...
async function handler(event, context) {
    try {
        console.log('Starting Add Attachment to PDF Process in Lambda');
        const inputPdfPath = event.inputPdfPath || INPUT_PDF_PATH;
        const attachmentFilePath = event.attachmentFilePath || ATTACHMENT_FILE_PATH;
        const outputPdfName = event.outputPdfName || OUTPUT_PATH;

        console.log(`Input PDF: ${inputPdfPath}`);
        console.log(`Attachment file: ${attachmentFilePath}`);
        console.log(`Output PDF with attachment: ${outputPdfName}`);
        console.log('Operation: Add attachment to PDF');

        const pdfBase64 = readAndEncodeFile(inputPdfPath);
        const attachmentBase64 = readAndEncodeFile(attachmentFilePath);
        const apiResponse = await addAttachmentToPdf(
            pdfBase64, 
            attachmentBase64, 
            path.basename(inputPdfPath), 
            path.basename(attachmentFilePath)
        );
        const result = await handleAsyncResponse(apiResponse, outputPdfName);

        if (result.status === 'success') {
            console.log('Attachment addition completed successfully!');
            return {
                statusCode: 200,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: true,
                    message: 'Attachment addition completed successfully',
            ...
            console.log(`Attachment addition failed: ${result.error}`);
            return {
                statusCode: 500,
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    success: false,
                    message: 'Attachment addition failed',
                    error: result.error
                })
            };
        }
...
async function main() {
    try {
        console.log('Starting Add Attachment to PDF Process (Local Test)');
        console.log('=== Adding Attachment to PDF Document ===');
        
        const inputPdfPath = INPUT_PDF_PATH;
        const attachmentFilePath = ATTACHMENT_FILE_PATH;
        ...
        console.error(`Attachment addition failed: ${err.message}`);
        process.exit(1);
    }
}
